...
//
//CATLGPUBABR04.java,v
//Revision 1.17  2008/01/30 19:39:16  wendy
...
//changes
//
//Revision 1.9  2006/09/27 16:03:30  joan
//fix compile
//
//Revision 1.8  2006/09/27 15:51:49  joan
//changes
//
//Revision 1.7  2006/09/26 18:42:07  joan
//working on catlgpub
//
//Revision 1.6  2006/09/26 01:23:56  joan
//fixes
//
//Revision 1.5  2006/09/26 00:55:44  joan
//add catupdatedby
//
//Revision 1.4  2006/09/19 22:02:16  joan
//changes
//
//Revision 1.3  2006/03/03 19:23:26  bala
//remove reference to Constants.CSS
//
//Revision 1.2  2006/01/24 17:01:03  yang
//Jtest Changes
//
...
/**
 * LSEOBUNDLE publish abr
 *
 *@author     Administrator
 */
public class CATLGPUBABR04 extends PokBaseABR {
 	// Class constants
  	private final static String ABR = "CATLGPUBABR04";
  ...
  /**
    ...
    public void execute_run() {
    	...
    	try {
			...
			String strCATLGPUBBNDLLASTRUN = m_utility.getAttrValue(m_ei, "CATLGPUBBNDLLASTRUN");
			if (strCATLGPUBBNDLLASTRUN == null || strCATLGPUBBNDLLASTRUN.length() <= 0) {
				OPICMList attList = new OPICMList();
				attList.put("CATLGPUBBNDLLASTRUN","CATLGPUBBNDLLASTRUN=" + STARTDATE);
				strCATLGPUBBNDLLASTRUN = STARTDATE;
				m_utility.updateAttribute(m_db, m_prof, m_ei, attList);
			} else {
				if (!m_utility.isDateFormat(strCATLGPUBBNDLLASTRUN)) {
					println("<p>CATLGPUBBNDLLASTRUN is not in date format 1980-01-01 or 1980-01-01-00.00.00.000000</p>");
					setReturnCode(FAIL);
				} else {
					if (strCATLGPUBBNDLLASTRUN.length() == 10) {
						OPICMList attList = new OPICMList();
						attList.put("CATLGPUBBNDLLASTRUN","CATLGPUBBNDLLASTRUN="+ strCATLGPUBBNDLLASTRUN + "-00.00.00.000000");
						strCATLGPUBBNDLLASTRUN =  strCATLGPUBBNDLLASTRUN + "-00.00.00.000000";
						m_utility.updateAttribute(m_db, m_prof, m_ei, attList);
					}
				}
			}

      		String strOFFCOUNTRY = m_utility.getAttrValue(m_ei, "OFFCOUNTRY");
      		...
      		if (getReturnCode() == PASS) {
				...
				pdg = new CATLGPUBLSEOBDLPDG(null, m_db, m_prof, domains);

        		pdg.setUpdatedBy(ABR + getRevision() + strNow);
        		pdg.m_strEntityType = "LSEOBUNDLE";
				pdg.setEntityItem(m_ei);
				pdg.setChunkSize(m_iChunk);
				pdg.setDoSearch(m_bSearch);
				if (m_iDays <= 0) {
        			pdg.executeAction(m_db, m_prof);
        			log(ABR+" finish generating data");
        			println("<!-- "+pdg.getDebugInfo()+"-->");

					OPICMList attList = new OPICMList();
					attList.put("CATLGPUBBNDLLASTRUN","CATLGPUBBNDLLASTRUN=" + strNow);

					m_utility.updateAttribute(m_db, m_prof, m_ei, attList);
				} else {
					println("<!-- "+ABR + " in using date range mode -->");
					log(ABR + " in using date range mode");
					String strLastRunAtt = "CATLGPUBBNDLLASTRUN";
					String strLastRunTime = strCATLGPUBBNDLLASTRUN;
					if (strLastRunTime.substring(0,4).equals("1980")) {
						strLastRunTime = m_utility.getEarliestTime(m_db, m_prof, "LSEOBUNDLE");
					}
	...
	*/
	public String getDescription() {
		return "Catalog Offering Publication For LSEOBUNDLE ABR";
	}
  ...
  /** removed because of jtest error: "Avoid hiding inherited "static" member methods. [OOP.AHSM]"
    ...
    return ("CATLGPUBABR04.java,v 1.17 2008/01/30 19:39:16 wendy Exp");
  }*/
	...
	*/
	public String getABRVersion() {
		return "CATLGPUBABR04.java "+getRevision();
	}
	...
	private void readPropertyFile()  {
    	...
    	if (bFoundProperties) {
			m_iDays = Integer.parseInt(properties.getProperty("CATLGPUBABR04.DAYS", "0"));
			m_iChunk = Integer.parseInt(properties.getProperty("CATLGPUBABR04.CHUNK", "100"));
			m_bSearch = properties.getProperty("CATLGPUBABR04.SEARCH", "false").equalsIgnoreCase("true");
		}
  	} //readPropertyFile
}
