...
public class WWCOMPATABR extends ADSABRSTATUS
{
	...
	static
	{CompatABR_TBL.put("MODEL", "COM.ibm.eannounce.abr.sg.adsxmlbh1.ADSCOMPATGENABR");}
...
//					 RQK check to see if MQ propfile name exists on root entity
					...
					setT2DTS(rootEntity, mqAbr, adsStatusHistoy, statusHistory, dtfs); // T2 timestamp
...
//				}
	...
	private String getDTFS(EntityItem rootEntity, XMLMQ mqAbr) {
		...
		if (STATUS_FINAL.equals(statusFlag)) {
			queuedvalue = getQueuedValue("COMPATGENABRSTATUS");
			
		} else {
			queuedvalue =  getRFRQueuedValue("COMPATGENABRSTATUS");
		}
...
//						 In case of it was queued before status was set to RFR. 
	...
	private AttributeChangeHistoryGroup getADSABRSTATUSHistory() throws MiddlewareException {
		String attCode = "COMPATGENABRSTATUS";
		EntityItem rootEntity = m_elist.getParentEntityGroup().getEntityItem(0);
		...
		if (att != null) {
			...
			addDebug(" COMPATGENABRSTATUS of "+rootEntity.getKey()+ "  was null");
			return null;
		}
	}
...
//========================================================================================
	...
	protected boolean fullMode(){
	        String result =  COM.ibm.opicmpdh.middleware.taskmaster.ABRServerProperties.getValue("COMPATGENABRSTATUS" , "_FullMode");
	        if (result.equals("true")||result.equals("True")||result.equals("TRUE")){
	        	addDebug("COMPATGENABRSTATUS is running in FullMode!");
	        	return true;
	        }else{
	        	addDebug("COMPATGENABRSTATUS is not running in FullMode!");
	        	return false;
	        }
	}
