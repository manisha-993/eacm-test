...
=	if("".equals(tmf_xml)) {
				...
				updateParkStatus = new UpdateParkStatus("MD_CHW_IERP", caller.getRFCNum());
				this.addDebug("Calling "+updateParkStatus.getRFCName());
				updateParkStatus.execute();
				this.addDebug(updateParkStatus.createLogEntry());
				if (updateParkStatus.getRfcrc() == 0) {
	...
	public void updateSalesBom(FCTRANSACTION fctransaction, Set<String> plnts, List<MODEL> models) throws Exception {
		for(String plant : plnts) {
			//call ChwBomCreate 
			ChwBomCreate chwBomCreate = new ChwBomCreate(fctransaction.getTOMACHTYPE()+"BOMUPG", plant);
			...
			// start lock
			String fileName = "./locks/FCTRANSACTION" + fctransaction.getTOMACHTYPE() + "BOMUPG" + plant + ".lock";
				...
				while (true) {
					try {
						...
						if (fileLock != null) {
							...
							ChwReadSalesBom chwReadSalesBom = new ChwReadSalesBom(fctransaction.getTOMACHTYPE()+"BOMUPG", plant);
							...
							if (componmentList != null && componmentList.size() > 0) {
								...
								for(MODEL model : models) {
									...
									if (hasMatchComponent(componmentList, componment)) {
										...
										//call ChwBomMaintain
										ChwBomMaintain chwBomMaintain = new ChwBomMaintain(model.getMACHTYPE()+"BOMUPG", plant, model.getMACHTYPE()+model.getMODEL(),POSNR,"SC_"+model.getMACHTYPE()+"_MOD_"+model.getMODEL());
						...
						this.addDebug("other abr is running createSalesBOMforType" + "BOMUPG");
						Thread.sleep(5000);
					}
				}
			}
