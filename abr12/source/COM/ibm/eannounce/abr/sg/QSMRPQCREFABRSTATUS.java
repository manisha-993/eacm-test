...
**********************************************************************************/
//$Log: QSMRPQCREFABRSTATUS.java,v $
//Revision 1.6  2016/11/28 21:12:12  mmiotto
//Fix for some attributes
//Added file handling - create in one dir, copy to ftp dir when complete
//
//Revision 1.5  2016/11/11 18:50:01  mmiotto
//Fix output file name
//
//Revision 1.4  2016/11/11 16:03:09  mmiotto
...
//Code cleanup
//
//Revision 1.1  2016/10/07 21:24:29  mmiotto
//1st commit. Working version.
//
 ...
 *
 */
public class QSMRPQCREFABRSTATUS extends PokBaseABR
{
    ...
    */
    ...
    public void execute_run()
    {
        ...
        try {
			...
			AttributeChangeHistoryGroup achgQSMRPQCREFABRSTATUS = getAttributeHistory("QSMRPQCREFABRSTATUS");
	        if (existBefore(achgQSMRPQCREFABRSTATUS, STATUS_PASSED)){
				strCREFIF = "1";
	        }else{
	        	strCREFIF = "0";
	        }
			...
			setDGRptName("QSMRPQCREFABRSTATUS"); // Set the report name
			setDGTitle(navName);
...
3.	'.txt'
    ...
    /*
     * Create records for FEATURE
     * @param rootEntity the FEATURE rootEntity this ABR runs on
	...
	private void createFeatureRecords(EntityItem rootEntity, OutputStreamWriter wOut) throws IOException, SQLException, MiddlewareException, ParseException {
		...
		String minGenAvailDate = getEarliestGENAVAILDATE(prodstructGrp);
		
		for (int pI = 0; pI < prodstructGrp.getEntityItemCount(); pI++){
			EntityItem eiProdstruct = prodstructGrp.getEntityItem(pI);

			String strAnnDate = PokUtils.getAttributeValue(eiProdstruct, "ANNDATE", ",", "", false);

			sb = new StringBuffer();
			sb.append(getValue(ISLMRFA, PokUtils.getAttributeValue(rootEntity, "FEATURECODE", ",", "", false)));
			...
			strDescription += PokUtils.getAttributeValue(rootEntity, "FEATURECODE", "", "", false);
			...
			sb.append(getValue(DSLMGAD, minGenAvailDate));
			sb.append(getValue(DSLMMES, minGenAvailDate));
			...
			String strAvailGenAreaSel = PokUtils.getAttributeValue(rootEntity, "GENAREASELECTION", "", "");
			String geoModGeo = "";
			...
			sb.append(getValue(TSLMDES, PokUtils.getAttributeValue(rootEntity, "INVNAME", "", "")));
			...
			EANFlagAttribute featGeoList = (EANFlagAttribute)rootEntity.getAttribute("GENAREASELECTION");
			if(featGeoList != null){
	...
	/**
	 * Return the min PRODSTRUCT.GENAVAILDATE
	 * @param prodstructGrp
	 * @return String
	 * @throws ParseException
	 */
	private String getEarliestGENAVAILDATE(EntityGroup prodstructGrp) throws ParseException{
		
		EntityGroup psGrp = prodstructGrp;
		...
		for (int i = 0; i < psGrp.getEntityItemCount(); i++){
			...
			strDate = PokUtils.getAttributeValue(eiProdstruct, "GENAVAILDATE", ",", "", false);
