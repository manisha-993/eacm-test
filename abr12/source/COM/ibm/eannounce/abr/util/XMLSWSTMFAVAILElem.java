...
public class XMLSWSTMFAVAILElem extends XMLElem {
	...
	public void addElements(Database dbCurrent, Hashtable table, Document document, Element parent,
		...
		ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1:parentItem: " + parentItem.getKey() + NEWLINE);
	    ...
	    //ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.addElements priorItem is " + (priorItem == null?" is null":"not null")+ " profileT1 valOn: " + priorItem.getProfile().getValOn()+ NEWLINE);
		...
		if (plnAvlVct.size() > 0) {
			...
			while (itr.hasNext()) {
				...
				if (ctryAudRec.isDisplayable() || ctryAudRec.isrfrDisplayable()) {
					...
					ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.addElements no changes found for " + ctryAudRec + NEWLINE);
				}
			...
			ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.addElements no planned AVAILs found" + NEWLINE);
		}
	}
	
	private void buildCtryAudRecs(TreeMap ctryAudElemMap, DiffEntity availDiff, StringBuffer debugSb) {
		ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs " + availDiff.getKey()+NEWLINE);
		...
		EntityItem prioritem = availDiff.getPriorEntityItem();
			...
			if (availDiff.isDeleted()) { // If the AVAIL was deleted, set Action = Delete
				...
				ABRUtil.append(debugSb,
						"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for deleted / update avail at T1: ctryAtt "
					...
					for (int im = 0; im < mfArray.length; im++) {
						...
						if (mfArray[im].isSelected()) {
							...
							if (ctryAudElemMap.containsKey(mapkey)) {
								...
								ABRUtil.append(debugSb,
										"WARNING buildCtryAudRecs for deleted " + availDiff.getKey() + " "
								...
								ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for deleted:" + availDiff.getKey() + " rec: "
				...
				ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for new avail:  ctryAtt and anncodeAtt "
					...
					for (int im = 0; im < mfArray.length; im++) {
						...
						if (mfArray[im].isSelected()) {
							...
							if (ctryAudElemMap.containsKey(mapkey)) {
								...
								ctryAudElemMap.put(ctryAudRec.getKey(), ctryAudRec);
								ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for new:" + availDiff.getKey() + " rec: "
									+ ctryAudRec.getKey() + NEWLINE);
							}
						}
					}
				...
				// else if one country in the countrylist has changed, update this row to update!
				...
				ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for curr avail: fAtt "
				...
				ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for prev avail:  fAtt "
					+ PokUtils.getAttributeFlagValue(prioritem, "COUNTRYLIST") + NEWLINE);
				if (fAtt != null && fAtt.toString().length() > 0) {
					// Get the selected Flag codes.
					MetaFlag[] mfArray = (MetaFlag[]) fAtt.get();
				...
				while (itr.hasNext()) {
					...
					if (!prevSet.contains(ctryVal)) { // If AVAIL.COUNTRYLIST has a country added, set that row's Action = Update
						...
						if (ctryAudElemMap.containsKey(mapkey)) {
							...
							ctryAudRec.setAction(UPDATE_ACTIVITY);
							ctryAudElemMap.put(ctryAudRec.getKey(), ctryAudRec);
							ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for added ctry:" + availDiff.getKey() + " rec: "
								+ ctryAudRec.getKey() + NEWLINE);
						}
					} else {
						...
						if (ctryAudElemMap.containsKey(mapkey)) {
							...
							CtryAudRecord ctryAudRec = new CtryAudRecord(availDiff, ctryVal);
							ctryAudElemMap.put(ctryAudRec.getKey(), ctryAudRec);
							ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for existing ctry:" + availDiff.getKey()
				...
				while (itr.hasNext()) {
					...
					if (!currSet.contains(ctryVal)) { //If AVAIL.COUNTRYLIST has a country deleted, set that row's Action = Delete
						...
						if (ctryAudElemMap.containsKey(mapkey)) {
							...
							ctryAudRec.setAction(DELETE_ACTIVITY);
							ctryAudElemMap.put(ctryAudRec.getKey(), ctryAudRec);
							ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.buildCtryAudRecs for delete ctry:" + availDiff.getKey() + " rec: "
								+ ctryAudRec.getKey() + NEWLINE);
						}
					}
				}
	...
	private Vector getPlanAvails(Hashtable table,StringBuffer debugSb){
		...
		ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.getPlannedAvails looking for AVAILTYPE:146 in AVAIL" + " allVct.size:"
		...
		for (int i = 0; i < allVct.size(); i++) {
			...
			if (diffitem.isDeleted()) {
				...
				ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.getPlannedAvails checking[" + i + "]: deleted " + diffitem.getKey()
				...
				ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.getPlannedAvails checking[" + i + "]:" + diffitem.getKey()
	...
	private static class CtryAudRecord extends CtryRecord{
		...
		private String[] derivePlannedavailability(DiffEntity parentDiffEntity, boolean findT1, StringBuffer debugSb) {
			...
			ABRUtil.append(debugSb,"XMLSWSTMFAVAILElembh1.derivePlannedavailability availDiff: " + (availDiff == null ? "null" : availDiff.getKey())
