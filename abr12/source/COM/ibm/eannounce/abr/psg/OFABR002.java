 ...
 *
 * $Log: OFABR002.java,v $
 * Revision 1.5  2008/01/30 20:00:55  wendy
 * Cleanup more RSA warnings
 *
 * Revision 1.4  2008/01/30 19:27:44  wendy
 * Cleanup RSA warnings
 *
 * Revision 1.3  2006/01/24 22:14:17  yang
 * Jtest changes
 *
 * Revision 1.2  2005/01/31 16:30:06  joan
 ...
 */
public final class OFABR002 extends PokBaseABR {
...
//	private static final String statusOrder = "001000400020";
	...
	public void execute_run() {
		...
		try {
			...
			//}
			//strSTATUS =	getAttributeFlagEnabledValue(strOF, iOF, "STATUS", "");
			...
			if ("0080".equals(strOFFERINGTYPE)
				&& "0050".equals(
					strOFPROJECTPHASE)) { // all status  and only phase when it is in plan plan
			...
			if (getReturnCode() == PASS) {
				...
				// Check 3A
				if (strPROJECTPHASE == null || strOFPROJECTPHASE == null) {
					setReturnCode(FAIL);
					rpt.append(msgs.getString("STATUS_ERROR"));
				} else if (
					phaseOrder.indexOf(strPROJECTPHASE)
					> phaseOrder.indexOf(strOFPROJECTPHASE)) {
					// Advance Phase
					String strFlagCode = null;
					if ("0020".equals(strBAVLFORSPECIALBID)) { // Yes
						// Advance to match Project
						strFlagCode = strPROJECTPHASE;
					} else {
						// Advance by one
						int iOrder = phaseOrder.indexOf(strOFPROJECTPHASE) + 4;
						strFlagCode = phaseOrder.substring(iOrder, iOrder + 4);
					}
					setControlBlock();
					setFlagValue("OFPROJECTPHASE", strFlagCode);
