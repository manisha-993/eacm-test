...
//
/**
 * <pre>Verifies Program Entity
 *
 * LS ABR11 to validate LSPRG (Program) information and linkages
 *
 ...
 *
 * Revision 1.17  2006/03/06 17:29:06  couto
 ...
 */
public class LSPRGABR11 extends PokBaseABR {
    ...
    public void execute_run() {
        try {
            ...
            sb.append(" Code: ");
            sb.append(getAttributeValue(getEntityType(), getEntityID(), "LSPRGID", DEF_NOT_POPULATED_HTML));
            sb.append("<br />");
            println(sb.toString());

            //if (checkFlagValues("LSPRGLIFECYCLE", XR_LSPRGLIFECYCLE_Available) ||
            //    checkFlagValues("LSPRGLIFECYCLE", XR_LSPRGLIFECYCLE_Retired))
            //{
            //    println("<br />LSPRGABR11 not processed.  Current Lifecycle Status is not appropriate for this action.<br />");
            //    iResults = FAIL;
            //}
            //else {
            ...
            if (iResults == PASS) {
                ...
                triggerWorkFlow("WFPRGAV");
                triggerLock("EXTLSPRGLOCK");
            }
            else {
                triggerWorkFlow("WFPRGFL");
            }
            //}
            sb = new StringBuffer();
    ...
    private boolean checkRelators() throws java.rmi.RemoteException, MiddlewareException, MiddlewareShutdownInProgressException, java.io.UnsupportedEncodingException {
        boolean result = true;
        // A Program must be linked to one and only one Division
        if (!checkRelator2("LSDIV", "LSDIVPRG", UNIQUE2, false)) {
        //if (!checkUniqueParent("LSDIV", "LSDIVPRG")) { // Behavior changed by LF
            result = false;
        }

        // there must be one and only one owner
        if (!checkRelator2("LSPER", "LSPRGPEROW", UNIQUE2, true)) {
        //if (!checkUniqueChild("LSPER", "LSPRGPEROW")) { // Behavior changed by LF
            result = false;
        }
