...
//
/**
 * <pre>Verifies Global Roadmap Entity
 *
 * LS ABR15 to validate LSGRM (Global Roadmap) information and linkages
 *
 ...
 *
 * Revision 1.17  2006/03/06 17:25:54  couto
 ...
 */
public class LSGRMABR15 extends PokBaseABR {
    ...
    public void execute_run() {
        try {
            ...
            sb.append(" Code: ");
            sb.append(getAttributeValue(getEntityType(), getEntityID(), "LSGRMID", DEF_NOT_POPULATED_HTML));
            sb.append("<br />");
            println(sb.toString());

            //if (checkFlagValues("LSGRMLIFECYCLE", XR_LSGRMLIFECYCLE_Available) ||
            //  checkFlagValues("LSGRMLIFECYCLE", XR_LSGRMLIFECYCLE_Retired))
            //{
            //  println("<br />LSGRMABR15 not processed.  Current Lifecycle Status is not appropriate for this action.<br />");
            //  iResults = FAIL;
            //}
            //else {
            ...
            if (iResults == PASS) {
                ...
                triggerWorkFlow("WFLSGRMAV");
                triggerLock("EXTLSGRMLOCK");
            }
            else {
                triggerWorkFlow("WFLSGRMFL");
            }
            //}
            sb = new StringBuffer();
    ...
    private boolean checkRelators() throws java.rmi.RemoteException, MiddlewareException, MiddlewareShutdownInProgressException, java.io.UnsupportedEncodingException {
        boolean result = true;
        // A Global roadmap must be linked to one and only one Division
        if (!checkRelator2("LSDIV", "LSDIVGRM", UNIQUE2, false)) {
        //if (!checkUniqueParent("LSDIV", "LSDIVGRM")) { // Behavior changed by LF
            result = false;
        }

        // there must be one and only one owner
        if (!checkRelator2("LSPER", "LSGRMPEROW", UNIQUE2, true)) {
        //if (!checkUniqueChild("LSPER", "LSGRMPEROW")) { // Behavior changed by LF
            result = false;
        }
    ...
    public String getDescription() {
        return "Verifies Global Roadmap Entity";
    }
