 ...
 * $Log: LSWWCCABR5.java,v $
 * Revision 1.14  2014/01/13 13:50:51  wendy
 * migration to V17
 *
 * Revision 1.13  2008/01/30 19:26:59  wendy
 * Cleanup RSA warnings
 *
 * Revision 1.12  2006/06/26 12:42:35  chris
 * moved call to EACustom.getTOUDiv()
 *
 * Revision 1.11  2006/03/13 19:05:46  couto
 * Fixed copyright info.
 *
 * Revision 1.10  2006/03/08 12:51:04  couto
 ...
 *
 * Revision 1.9  2006/01/26 00:36:19  joan
 * Jtest
 *
 * Revision 1.8  2006/01/22 18:28:30  joan
 * changes for Jtest
 *
 * Revision 1.7  2005/01/26 23:37:00  joan
 * changes for Jtest
 *
 ...
 */
public class LSWWCCABR5 extends PokBaseABR {
    ...
    public void execute_run() {
        try {
            ...
            println("<b>Volume Price Display Order Check:</b>" + NEW_LINE);
            m_bOutputAsList = true;
            
            bDisplayOrd = checkVPCDisplayOrder();
            m_bOutputAsList = false;
            vctVPC = getChildrenEntityIds(getEntityType(), getEntityID(), "LSVPC", "LSWWCCVPC");
            it = vctVPC.iterator();
            
            while (it.hasNext()) {
                Integer pdhVPC = (Integer) it.next();
                String strLSCRSVPCSTATUS = getAttributeFlagEnabledValue("LSVPC", pdhVPC.intValue(), "LSCRSVPCSTATUS","");
                if ("0010".equals(strLSCRSVPCSTATUS)) {
                    isFinal = false;
                }
            }
            if (!isFinal || !bDisplayOrd) {
                triggerWorkFlow("WFLSWWSCCCSREAVA");
                if (!isFinal) {
                    printErrorMessage(
                        ERR_IAB2021E,
                        new String[]{
            ...
            if ("0030".equals(strPricing) && ("0020".equals(strCAEdit) || "0050".equals(strCAEdit))) {
                if (isFinal && bDisplayOrd) {
                ...
                if (!"0030".equals(strPricing)) {
                    printMessage(
                        ...
                        new String[] {
                            getAttributeMetaFlagDescription(getRootEntityType(),"LSWWCCPRICINGSTATUS", strPricing)
                        });
                }
            }
