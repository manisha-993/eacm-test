 ...
 */
public class ECCMCATLGPUBABR03 extends PokBaseABR {
    /**
    *  Execute ABR.
    *
    */
    ...
    // Class constants
    public final static String ABR = new String("ECCMCATLGPUBABR03");
    private final static String ATT_LASTRUN = new String("CATLGCVARPUBLASTRUN");
    private final static String ATT_OFFCOUNTRY = new String("OFFCOUNTRY");
    private final static String QUEUE = new String("ECCMCATLGPUBABR07");
    ...
    public void execute_run() {
        ...
        if (getReturnCode() == PASS) {
			String strGA = aGA[0];
	       	...
	       	String[] aeiCVAR = getCVARs(m_db, m_prof, strGA);

			for (int i=0; i < aeiCVAR.length; i++) {
				String strEI = aeiCVAR[i];
			...
			removeCATCVAR (m_db, m_prof, "CATCVAR") ;
            OPICMList attList = new OPICMList();
	...
	private String[] getCVARs(Database _db, Profile _prof, String _strGA) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		// Initialize some SP specific objects needed in this method
		String strTraceBase = " ECCMCATLGPUBPDG getCVARs method";
		ResultSet rs = null;
		...
		/*
		CVAR was last updated since the last run for this CATLGCNTRY.CATLGCVARPUBLASTRUN
		TARGANNDATE_CVAR < NOW() + 15 Days “logical OR” STATUS_CVAR = Ready for Review (0040)
		...
		*/
		...
		String strLastRun = m_utility.getAttrValue(m_ei, "CATLGCVARPUBLASTRUN");
        try {
		...
		aeiReturn = new String[returnList.size()];
		vReturn.toArray(aeiReturn);
		return aeiReturn;
	}

	private void removeCATCVAR (Database _db, Profile _prof, String _strEntityType) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		/*
		Instances of CATCVAR should be removed whenever the instance has:
		not been updated for the past 30 days
		and PUBTO < NOW() + 30 days
		*/
		String strTraceBase = "ECCMCATLGPUBPDG removeCATCVAR method " + _strEntityType;
  ...
  */
  ...
  /**
    ...
    return ("$Id: ECCMCATLGPUBABR03.java,v 1.14 2008/01/30 19:27:19 wendy Exp $");
  }
