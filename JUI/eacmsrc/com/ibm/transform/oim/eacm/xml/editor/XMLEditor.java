...
public class XMLEditor extends JPanel implements XMLEditorGlobals, PropertyChangeListener
{
    ...
    private Hashtable myActions= new Hashtable(), dtdActions=new Hashtable();
    ...
    private Hashtable origPulldownMenuItems = new Hashtable();
    private Hashtable origPopupMenuItems = new Hashtable();
        ...
        if (dictionary !=null)
        {
            try
            {
                ...
                */
                ...
                spellCheckerHandler = new XMLJSpellHandler(spellParser,wrapper, editor);
                spellCheckerHandler.setFrame(rootPane.getTopLevelAncestor());
                spellCheckerHandler.init();
...
//        add(getToolBar(dictionary, actions),"North");
    ...
    */
    private JMenuBar createMenuBar(XMLEditorMenuListener menuListener, Hashtable actions)
    {
        ...
        Vector origmi = new Vector(VIEW_SPEC.length);
        ...
        origmi = new Vector(TEXT_SPEC.length);
        for (int i=0; i<TEXT_SPEC.length; i++)
        {
...
*/
        ...
        theBar.add(menu);
        origmi = new Vector(HTML_SPEC.length);
        for (int i=0; i<HTML_SPEC.length; i++){
    ...
    private void addActions(Hashtable actions, Action a)
    {
...
/*markup test
        // markup
//fixme should these be in the actions group.. should they be disabled to start?
        // markup
        actionsArray = markupMgr.getActions();
        for (int i = 0; i < actionsArray.length; i++)
        {
            a = actionsArray[i];
            actions.put(a.getValue(Action.NAME), a);
...
*/
    ...
    */
    private void createPopupMenu(XMLEditorMenuListener menuListener, Hashtable actions)
    {
        ...
        popupMenu.add(menu);
        origmi = new Vector(VIEW_SPEC.length);
        for (int i=0; i<VIEW_SPEC.length; i++){
            origmi.addElement(VIEW_SPEC[i].getMenuItem());}
...
//Find/repl
        addToolBarButton(bar,actions,FIND_REPL_ACTION, "Find/Replace", "/images/findit.gif",null);

//      if (dictionary != null)
        addToolBarButton(bar,actions,SPELLCHECK_ACTION, "Spell check", "/images/SpellCheck.gif",null);
        ...
        addToolBarButton(bar,actions,INSERT_BR_ACTION, "Line Break", "/images/Break.gif","br");
...
//System.err.println("XMLEditor::appendKey keyevent.char "+c);
    ...
    */
    ...
    private void setSupportedItems()
    {
        ...
        String fileMenuText = "File";//XMLEditorMenuSpec.getMenuText("File");
        String helpMenuText = "Help";//XMLEditorMenuSpec.getMenuText("Help");
        Vector removedMenus = new Vector(1);
        String elemArray[];
        JMenuBar theBar;
        JToolBar bar = null;
    ...
    */
    private class ExitListener extends WindowAdapter implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            {
                ((Window)rootPane.getTopLevelAncestor()).hide();
