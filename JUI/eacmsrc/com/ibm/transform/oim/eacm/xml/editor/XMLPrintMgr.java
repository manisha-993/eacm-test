    ...
    */
                ...
                printThread = new Thread()
                {
                    public void run()
                    {
                        try {
                            ...
                            dialog = new PrintPreviewDialog(printJob,editor);
                            dialog.show();
                            dialog.dispose();
                            dialog.dereference();
                            dialog=null;
    ...
    //          });
        ...
        private class PrintPreviewDialog extends JDialog implements ActionListener, KeyEventPostProcessor
        {
            ...
            private static final int PAGE_W = 710;
            private static final int PAGE_H = 548;

            PrintPreviewDialog(PrinterJob pj, JEditorPane editor)
            {
...
//System.err.println("canvas size: "+canvas.getSize());
        ...
        private class PrintPreviewCanvas extends JPanel
        {
        	private static final long serialVersionUID = 1L;
...
//System.err.println("PrintPrev:paint pageW: "+pageW+" pageH: "+pageH+" canvasW: "+canvasW+" canvasH: "+canvasH);
                ...
                dxoff = new Double(xoff);
                dyoff = new Double(yoff);
                // position 'paper' on the preview canvas
//                    g2.translate((float)xoff, (float)yoff);
                g2.translate(dxoff.floatValue(), dyoff.floatValue());

                dscale = new Double(scale);
//                    g2.scale((float)scale, (float)scale);
                g2.scale(dscale.floatValue(), dscale.floatValue());
                ...
                try
                {
//System.err.println("**************PRINTPREVIEW calling print()");
