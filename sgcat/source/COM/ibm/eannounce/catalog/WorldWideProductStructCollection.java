 ...
 *
 * How this collection is derived is expressed in the WorldWideProductCollectionId
 *
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class WorldWideProductStructCollection
  ...
  public static final String TMF_VE1 = "TMPS1";
  public static final String SEO_VE1 = "WWSEOPS1";
  //Added by Guo Bin, 2007-8-03
  private static String SP_SUFFIX = "K";
  /**
   ...
   * @param _cid
   */
  public WorldWideProductStructCollection(WorldWideProductCollectionId _cid) {
    super(_cid);
  }
  ...
  public static void main(String[] args) {
    ...
    if (args[0].equals("1")) {
      WorldWideProductStructCollection.syncToCatDb();
    }
    else if (args[0].equals("2")) {
      WorldWideProductStructCollection.idlToCatDb();
    }
  }
  ...
  public final String dump(boolean _b) {
    ...
    sb.append(NEW_LINE + "\tCatId is:" + this.getWorldWideProductCollectionId());
    sb.append(NEW_LINE + "\t---------");
  ...
  public void get(Catalog _cat) {
    ...
    D.ebug(D.EBUG_SPEW,
           "get() - here is wwcpid..." + ":isByInteval:" + wwpcid.isByInterval() +
    ...
    try {
      ...
      if (wwpcid.isByInterval() && wwpcid.isFromPDH()) {
        ...
        if (wwpcid.isFromCAT() && wwpcid.isNetChanges()) {
          try {
            rs = db.callGBL4015(new ReturnStatus( -1), strEnterprise, iNLSID,
                                wwpcid.getWWPartNumber() + "%");
          ...
          for (int i = 0; i < rdrs.size(); i++) {
            ...
            //
            WorldWideProductId wwpid = new WorldWideProductId(strEntityType, iEntityID, gami, wwpcid);
  ...
  public void put(Catalog _cat, boolean _bcommit) {
    ...
    WorldWideProductCollectionId wwpcid = this.getWorldWideProductCollectionId();
    ...
    if (wwpcid.hasCollateralId() && wwpcid.isFromPDH() && wwpcid.isFullImages()) {
      ...
      D.ebug(D.EBUG_SPEW,
             ...
             ":isFromPDH: " + wwpcid.isFromPDH() + ":isFullImages:" +
             wwpcid.isFullImages() + ":hasCollateralId:" +
             wwpcid.hasCollateralId() + ":colid:" + wwpcid.getCollateralId());
      ...
      while (it.hasNext()) {
        ...
        try {
          db.callGBL8979(new ReturnStatus( -1), strEnterprise, strWWEntityType,
                         iWWEntityID, strCollEntityType,
    ...
    if (wwpcid.hasCollateralId()) {
	  ...
	  for (int x = smc.getCount()-1; x >=0; x--) {
        ...
        if (smi.getChildType().equals(this.SEO_ENTITYTYPE) &&
          ...
          WorldWideProductId wwpid = new WorldWideProductId(smi.getChildType(),
              smi.getChildID(), gami, wwpcid);
        ...
        else if ( (smi.getChildType().equals(this.TMF_ENTITYTYPE) &&
                   smi.getChildLevel() == 0)) {
          D.ebug(D.EBUG_SPEW,
        ...
        else if ( (smi.getChildType().equals("LSEOBUNDLE") &&
                   smi.getChildLevel() == 0)) {
          D.ebug(D.EBUG_SPEW, "processSyncMap has Collateral ID LSEOBUNDLE...");
          WorldWideProductId wwpid = new WorldWideProductId(smi.getChildType(),
              smi.getChildID(), gami, wwpcid);
          WorldWideProduct wwp = (WorldWideProduct)this.get(wwpid);
          if (wwp == null) {
      ...
      D.ebug(D.EBUG_SPEW, "processSyncMap has WWCOMPONENT ID TMF_ENTITYTYPE 1");
      for (int x = 0; x < smc.getCount(); x++) {
        SyncMapItem smi = smc.get(x);
        if (smi.getRootType().equals(this.SEO_ENTITYTYPE)) {
          D.ebug(D.EBUG_SPEW,
                 "processSyncMap has WWCOMPONENT ID SEO_ENTITYTYPE 1");
          WorldWideProductId wwpid = new WorldWideProductId(smi.getRootType(),
              smi.getRootID(), gami, wwpcid);
          WorldWideProduct wwp = (WorldWideProduct)this.get(wwpid);
          if (wwp == null) {
            wwp = new WorldWideProduct(wwpid);
            wwp.setSmc(new SyncMapCollection());
      ...
      //
      // ok.. we have to distribute the smc's down to the ww product itself
      //
      for (int x = 0; x < smc.getCount(); x++) {
        SyncMapItem smi = smc.get(x);
        D.ebug(D.EBUG_SPEW,
               "processSyncMap:Processing Item:" + x + ":" + smi.dump(false));
        WorldWideProductId wwpid = new WorldWideProductId(smi.getRootType(),
            smi.getRootID(), gami, wwpcid);
        WorldWideProduct wwp = (WorldWideProduct)this.get(wwpid);
        if (wwp == null) {
  ...
  public static void syncToCatDb() {
    ...
    D.ebug(D.EBUG_SPEW,"WorldWideProductStructCollection.syncToCatDb");
    //
...
//    }
    ...
    try {
	  ...
	  Class tclass = WorldWideProductStructCollection.class;
      ...
      for (int icycle = 0; icycle < 2; icycle++) {
        ...
        if (icycle == 0) {
            ...
            strVE = WorldWideProductStructCollection.SEO_VE1;
            //smcOuter = new SyncMapCollection(cat, strEntityType, strVE, cati, 9);
        }
        else {
          // TMF section
          strEntityType = WorldWideProductCollection.TMF_ENTITYTYPE;
          strVE = WorldWideProductStructCollection.TMF_VE1;
          //smcOuter = new SyncMapCollection(cat, strEntityType, strVE, cati, 9);
        }
        //add by houjie&liubing 2007-08-28, get configurable NLSID information for WorldWideProduct,ProdStruct
        int iNLSSave[]=new int[2];
        iNLSSave[0] = CatalogProperties.getSaveNLSID(WorldWideProductStructCollection.class,WorldWideProduct.class, strVE);
        iNLSSave[1] = CatalogProperties.getSaveNLSID(WorldWideProductStructCollection.class,ProdStruct.class, strVE);
        D.ebug(D.EBUG_SPEW,"WorldWideProductStructCollection.WorldWideProduct."+strVE+"_nlssave="+iNLSSave[0]);
        D.ebug(D.EBUG_SPEW,"WorldWideProductStructCollection.ProdStruct."+strVE+"_nlssave="+iNLSSave[1]);
        //
        ...
        for (int x = 0; x < rdrs.size(); x++) {
          ...
          while (en.hasMoreElements()) {
            ...
            for (int i = 0; i < vct.size(); i++) {
              ...
              D.ebug(D.EBUG_SPEW,
                  "WorldWideProductStructCollection gami/prof check:nlsi.getNLSID()=" +
                  nlsi.getNLSID() +
                  ",gami.getNLSID()=" + gami.getNLSID());
              if (nlsi.getNLSID() == gami.getNLSID()) {
                D.ebug(D.EBUG_SPEW,
                    "WorldWideProductStructCollection gami/prof check: setting read language to:" +
                    nlsi.getNLSID());
            ...
            WorldWideProductCollectionId wwpcid = new
                WorldWideProductCollectionId(cati, gami, CollectionId.PDH_SOURCE,
                                             CollectionId.NET_CHANGES);

            WorldWideProductStructCollection wwpc = new WorldWideProductStructCollection(
                wwpcid);
            ...
            while (it.hasNext()) {
              ...
              D.ebug(D.EBUG_SPEW, "Deactivating structure for " + wwpid.dump(false));
        	  ...
        	  //add by houjie&liubing 2007-08-28 : check NLSID for WorldWideProduct
        	  ...
        	  if(iNLSSave[0]==0 || iNLSSave[0]==gami.getNLSID()){
          		...
          		isWWPLoaded=false;
                  D.ebug(D.EBUG_SPEW,
                          ">>Skipping WWP save for " + wwp.getId() + ".  Targeting NLSID of " +
                          iNLSSave[0] + " not " + gami.getNLSID());
              }
        	  
        	  //add by houjie&liubing 2007-08-28 : check NLSID for ProdStructCollection
        	  if(isWWPLoaded && (iNLSSave[1]==gami.getNLSID() || iNLSSave[1]==0)){
        		  wwp.getReferences(cat, WorldWideProduct.PRODSTRUCT_REFERENCE);
                  //
                  // DWB .. maybe we can pull when needed
                  //
                  ProdStructCollection psc = wwp.getProdStructCollection();
                  D.ebug(D.EBUG_SPEW,
                         "syncToCatDb() ProdStructCollection size is :" + psc.size());
                  ...
                  D.ebug(D.EBUG_SPEW,
                          ">>> Skipping ProdStruct Save for " + wwp.getId() +
                          ".  Only Targeting NLSID of " +
                          iNLSSave[1] + "(not " + gami.getNLSID() + ")");     		  
        	  }    	 
        	  
        	  //add by houjie&liubing 2007-08-28 : no need to check NLSID for WorldWideAttributeCollection
              ...
              while (it2.hasNext()) {
                WorldWideAttribute wwa = (WorldWideAttribute) it2.next();
                wwa.get(cat);
                //
                // we would normally compare here
                // before we do a put
                ...
                //
                //
                // GAB: HERE'S WHERE OUR BASICRULE PROCESSING GOES FOR A GIVEN ***WWATTRIBUTE***.
                //   - Assume: rules are per (item)EntityType/(item)EntityId in our WWATTRIBUTE table.
                //   - we can mfg a wwattribute per ea. basicrule and put()
                //
                BasicRuleCollection brc = cat.getBasicRuleCollection(
                    "WWATTRIBUTES", gami);
                WorldWideAttribute[] awwa = brc.buildWorldWideAttributes(cat, wwp);
                D.ebug(D.EBUG_SPEW,
                ...
                for (int i = 0; i < awwa.length; i++) {
                  ...
                  wwac.put(wwa);
                  //wwa.get(cat); // nope, get's are done through basicrule logic.
                  wwa.put(cat, false);
                }
                //
                // GAB end basicrules prcessing for this worldwideattribute.
                //
...
//      Let delete the temptables by sessionID
  ...
  public static void parallelSyncToCatDb() {
    ...
    try {
      ...
      for (int icycle = 0; icycle < 2; icycle++) {
        ...
        //add by houjie&liubing 2007-09-13, get configurable NLSID information for WorldWideProduct,ProdStruct
        int iNLSSave[]=new int[2];
        iNLSSave[0] = CatalogProperties.getSaveNLSID(WorldWideProductStructCollection.class,WorldWideProduct.class, strVE);
        iNLSSave[1] = CatalogProperties.getSaveNLSID(WorldWideProductStructCollection.class,ProdStruct.class, strVE);
        D.ebug(D.EBUG_SPEW,"WorldWideProductStructCollection.WorldWideProduct."+strVE+"_nlssave="+iNLSSave[0]);
          ...
          while (en.hasMoreElements()) {
            ...
            while (it.hasNext()) {
              ...
              D.ebug(D.EBUG_SPEW,
                     "syncToCatDb() .. getting wwp and its collections... " +
                     wwp.toString());
        	  //add by houjie&liubing 2007-09-13 : check NLSID for WorldWideProduct
        	  boolean isWWPLoaded=false;
        	  if(iNLSSave[0]==0 || iNLSSave[0]==gami.getNLSID()){
        		  wwp.get(cat);
        	  ...
        	  //add by houjie&liubing 2007-09-13 : check NLSID for ProdStructCollection
        	  if(isWWPLoaded && (iNLSSave[1]==gami.getNLSID() || iNLSSave[1]==0)){
        		  wwp.getReferences(cat, WorldWideProduct.PRODSTRUCT_REFERENCE);
                  //
                  // DWB .. maybe we can pull when needed
              ...
              //add by houjie&liubing 2007-09-13 : do not config NLSID for WorldWideAttribute
              wwp.getReferences(cat, WorldWideProduct.ATTRIBUTE_REFERENCE);
              //
              // ok.. lets do some ww attributes
              //
  ...
  public void deactivateStructure(WorldWideProductId _wwpid, Catalog _cat,
    ...
    /*
         Call the sp to deactivate all children (key is Model or  wwseo)
         A wwproduct has Prodstruct and WWAttributes as children
      A prodstruct is a relationship betw MODEL and FEATURE.
      so when a WWPRODUCT  (MODEL) is deactivated, that is not handled
      in this class, it is handled in the ProductCollection class
     */
    try {
      dbe.callGBL4025(new ReturnStatus( -1), strEnterprise, strWWProdEType,
                     iWWProdEId, strCountryCode, strLanguageCode,
                     iNLSID);
      if (_bcommit) {
        db.commit();
      }
  ...
  public static void idlToCatDb() {
    ...
    ReturnDataResultSet rdrs = null;
    ResultSet rs = null;
    D.ebug(D.EBUG_SPEW,"WorldWideProductStructCollection.idlToCatDb");
    //
    // Lets try to clean up the temp tables first
    //
//    try {
//        Catalog.execUnixShell(CatalogProperties.getIDLTempTableCleanUpScript(WorldWideProductStructCollection.class));
//    }
//    catch (InterruptedException ex1) {
//        ex1.printStackTrace();
//        System.exit( -1);
...
//    }  
    ...
    try {
      ...
      Class mclass = WorldWideProductStructCollection.class;
      String strGeneralArea = CatalogProperties.getGeneralAreaKey(mclass);
      Catalog cat = new Catalog();
      String strEnterprise = cat.getEnterprise();
      Database db = cat.getCatalogDatabase();
      ...
      for (int icycle = 0; icycle < 2; icycle++) {
        ...
        for (int x = 0; x < rdrs.size(); x++) {
          ...
          while (en.hasMoreElements()) {
            ...
            WorldWideProductCollectionId wwpcid = new
                WorldWideProductCollectionId(cati, gami,
            ...
            while (it.hasNext()) {
              ...
              D.ebug(D.EBUG_SPEW, "idlToCatDb() .. getting wwp and its collections ... " + wwp.toString());
              ...
              if(!CatalogRunner.isBasicRuleLoad()) {
            	  ...
            	  if(iNLSSave[0]==0 || iNLSSave[0]==gami.getNLSID()){
	                  ...
	                  // Now.. lets
	                  //
	                  D.ebug(D.EBUG_SPEW,"idlToCatDb() .. updating the wwp to the CatDb... " + wwp.toString());
	                  wwp.put(cat, false);
	                  isWWPLoaded=true;	                  
              	  }else {
              		isWWPLoaded=false;
            	  ...
            	  if(isWWPLoaded && (iNLSSave[1]==gami.getNLSID() || iNLSSave[1]==0)){
                      ...
                      //
                      ProdStructCollection psc = wwp.getProdStructCollection();
                      D.ebug(D.EBUG_SPEW,"idlToCatDb() ProdStructCollection size is :" + psc.size());
                      Iterator it2 = psc.values().iterator();
                      //
                      // !!DWB
                      // Here is where we get to blow this stuff away first
                  ...
                  D.ebug(D.EBUG_SPEW,"idlToCatDb() WorldWideAttributeCollection size is :" + wwac.size());
                  Iterator it2 = wwac.values().iterator();
                  while (it2.hasNext()) {
                      WorldWideAttribute wwa = (WorldWideAttribute) it2.next();
                      wwa.get(cat);
              ...
              //   - we can mfg a wwattribute per ea. basicrule and put()
              //
              if(wwac == null && !CatalogRunner.isBasicRuleLoad()) {
				  wwac = wwp.getWorldWideAttributeCollection();
			  }
              if(CatalogRunner.isBasicRuleLoad()) {
            	  //add by houjie&liubing 2007-08-28 : check NLSID for WorldWideProduct
              ...
              BasicRuleCollection brc = cat.getBasicRuleCollection("WWATTRIBUTES", gami);
              WorldWideAttribute[] awwa = brc.buildWorldWideAttributes(cat, wwp);
              D.ebug(D.EBUG_SPEW,"idlToCatDb() WWAttributeArray from BasicRulesCollection size is :" + awwa.length);
              for (int i = 0; i < awwa.length; i++) {
                WorldWideAttribute wwa = awwa[i];
                wwac.put(wwa);
                //wwa.get(cat); // nope, get's are done through basicrule logic.
...
/*
 * 
 * $Log: WorldWideProductStructCollection.java,v $
 * Revision 1.2  2011/05/05 11:21:33  wendy
 * src from IBMCHINA
 *
 * Revision 1.6  2007/11/14 02:25:43  liubdl
 * *** empty log message ***
 *
 * Revision 1.5  2007/10/29 09:32:20  jingb
 * Change code in method syncToCatDb():
 * delete the temp table data by sessionID
 *
 * Revision 1.4  2007/10/19 02:26:06  jingb
 * *** empty log message ***
 *
 * Revision 1.3  2007/10/17 09:28:31  jingb
 * *** empty log message ***
 *
 * Revision 1.2  2007/10/16 05:40:45  jingb
 * *** empty log message ***
 *
 * Revision 1.1  2007/09/19 05:50:07  jingb
 * added by guobin 20070919
 *
 * 
 *
 * Revision 1.9  2007/09/19 08:48:31  GuoBin
 * Copy WorldWideProductCollection to WorldWideProductStructCollection
 * Change VE name,split WorldWideProductStructCollection from WorldWideProductCollection
 * 
 * Revision 1.8  2007/09/13 08:36:56  jiehou
 * *** empty log message ***
 * 
 * Revision 1.7  2007/09/13 05:54:53  sulin
 * no message
 *
 * Revision 1.6  2007/09/10 06:45:43  sulin
 * no message
 *
 * Revision 1.5  2007/09/03 03:11:31  jiehou
 * *** empty log message ***
 *
 * Revision 1.4  2007/08/31 06:41:56  jiehou
 * *** empty log message ***
 *
 * Revision 1.3  2007/08/30 06:25:58  jiehou
 * *** empty log message ***
 *
 * Revision 1.2  2007/08/29 06:00:30  jiehou
 * *** empty log message ***
 *
 * Revision 1.1.1.1  2007/06/05 02:09:34  jingb
 * no message
 *
 * Revision 1.22  2007/05/15 18:10:34  rick
 *  changed to run IDL logic gets in basic rule load path
 *
 * Revision 1.21  2006/10/16 18:18:08  gregg
 * childtype = MODEL, level==0.
 *
 * Revision 1.20  2006/09/21 20:58:44  gregg
 * prepping for BasicRuleLoad
 *
 * Revision 1.19  2006/09/14 23:04:02  gregg
 * reverse smc loop in processsyncmap to really order by OFF/ON
 *
 * Revision 1.18  2006/09/14 22:04:13  gregg
 * trying to see if we can "fix" processSyncMap wrt IMG/SEO
 *
 * Revision 1.17  2006/09/14 21:49:33  gregg
 * debug
 *
 * Revision 1.16  2006/08/28 20:07:26  dave
 * ok.. minor memory fixes
 *
 * Revision 1.15  2006/08/24 20:21:37  gregg
 * no smcOuter.clearAll at end of nls loop!
 *
 * Revision 1.14  2006/08/10 21:33:57  gregg
 * Use new SyncMapCollection constructor for chunky munky
 *
 * Revision 1.13  2006/08/08 23:30:26  gregg
 * little database switch up fixing
 *
 * Revision 1.12  2006/08/07 21:50:38  gregg
 * chunky munky for delta
 *
 * Revision 1.11  2006/07/11 18:14:17  gregg
 * taking back debugs to spew
 *
 * Revision 1.10  2006/06/29 21:31:38  gregg
 * BasicRule processing in sync.
 *
 * Revision 1.9  2006/06/27 22:41:31  gregg
 * null ptr fix
 *
 * Revision 1.8  2006/06/22 22:51:07  gregg
 * one more fix
 *
 * Revision 1.7  2006/06/22 22:50:31  gregg
 * compile phicks
 *
 * Revision 1.6  2006/06/22 22:48:21  gregg
 * changing chunk size and propertizing chunk size
 *
 * Revision 1.5  2006/06/20 02:49:09  dave
 * need to reset enum between cycles
 *
 * Revision 1.4  2006/06/20 02:42:26  dave
 * memory cleanup
 *
 * Revision 1.3  2006/06/20 01:12:13  dave
 * test reverse
 *
 * Revision 1.2  2006/06/19 23:39:39  gregg
 * use icycle in syncToCatDb
 *
 * Revision 1.1.1.1  2006/03/30 17:36:31  gregg
 * Moving catalog module from middleware to
 * its own module.
 *
 * Revision 1.101  2006/02/27 22:41:47  dave
 * added LSEOBUNDLE from
 * wwproduct colleciton to handle multi-media
 *
 * Revision 1.100  2006/02/21 22:41:41  gregg
 * more of said debugs
 *
 * Revision 1.99  2006/02/21 21:37:06  gregg
 * gami debugs
 *
 * Revision 1.98  2005/12/15 22:09:47  gregg
 * BasicRule building logic for WorldWideAttributes!!
 *
 * Revision 1.97  2005/12/01 23:44:45  joan
 * fixes
 *
 * Revision 1.96  2005/11/30 22:12:47  bala
 * deactivate prodstructcollection structure
 *
 * Revision 1.95  2005/11/29 19:19:48  dave
 * formatting
 *
 * Revision 1.94  2005/11/28 16:04:52  bala
 * change syncToCatdb - add debug statements
 *
 * Revision 1.93  2005/11/28 00:58:50  bala
 * <No Comment Entered>
 *
 * Revision 1.92  2005/11/18 23:26:57  bala
 * syncToCatDB - reset the children before updates
 *
 * Revision 1.91  2005/10/26 00:57:14  dave
 * <No Comment Entered>
 *
 * Revision 1.90  2005/10/25 07:05:49  dave
 * memory cleanup
 *
 * Revision 1.89  2005/10/25 04:09:24  dave
 * curtailing smc's to spew
 *
 * Revision 1.88  2005/10/25 02:52:21  dave
 * commenting out some debugs
 *
 * Revision 1.87  2005/10/14 21:37:22  gregg
 * fix null ptr
 *
 * Revision 1.86  2005/10/12 12:04:24  dave
 * newest way to chunk
 *
 * Revision 1.85  2005/10/04 23:24:16  joan
 * work on component
 *
 * Revision 1.84  2005/10/04 20:35:30  joan
 * fixes
 *
 * Revision 1.83  2005/10/04 16:28:14  joan
 * fixes
 *
 * Revision 1.82  2005/10/04 16:22:06  joan
 * add component
 *
 * Revision 1.81  2005/10/04 16:04:59  joan
 * fixes
 *
 * Revision 1.80  2005/10/03 22:08:33  joan
 * work on component
 *
 * Revision 1.79  2005/09/20 03:02:48  dave
 * starting to clean up collateral collection
 *
 * Revision 1.78  2005/09/13 05:52:24  dave
 * setting full idl run
 *
 * Revision 1.77  2005/09/13 04:14:31  dave
 * ok.. lets add the PROJCDNAM, and lets take a hard look at
 * WWSEO and how to get prod structures
 *
 * Revision 1.76  2005/09/13 03:09:32  dave
 * cleanup and more trace
 *
 * Revision 1.75  2005/09/13 02:53:47  dave
 * putting some headlights on the SMC
 *
 * Revision 1.74  2005/09/13 02:24:58  dave
 * ok change conflict fixed
 *
 * Revision 1.73  2005/09/13 01:58:22  dave
 * testing out the wwseo
 *
 * Revision 1.72  2005/09/12 03:53:45  dave
 * fixing sql parms
 *
 * Revision 1.71  2005/09/12 03:40:53  dave
 * fix to looping
 *
 * Revision 1.70  2005/09/12 02:01:49  dave
 * attempting to chunk out idl
 *
 * Revision 1.69  2005/09/12 01:34:21  dave
 * backing off IDL initial load
 *
 * Revision 1.68  2005/09/12 00:12:24  dave
 * new sql file to quickly ensure everything is accurate
 * in our catdb setup for data
 *
 * Revision 1.67  2005/07/07 19:36:58  bala
 * update WorldWideComponent catalog table
 *
 * Revision 1.66  2005/07/07 18:43:09  bala
 * plug for WorldWideComponents
 *
 * Revision 1.65  2005/06/23 03:00:43  dave
 * iterator probwem
 *
 * Revision 1.64  2005/06/23 02:42:42  dave
 * minor changes
 *
 * Revision 1.63  2005/06/22 21:17:16  gregg
 * change signature for processSyncMap
 *
 * Revision 1.62  2005/06/17 01:26:56  dave
 * some isPending fixes
 *
 * Revision 1.61  2005/06/17 01:05:26  dave
 * tying it all together
 *
 * Revision 1.60  2005/06/16 16:46:05  dave
 * changes for Collateral
 *
 * Revision 1.59  2005/06/13 04:54:40  dave
 * missed a processSyncMap
 *
 * Revision 1.58  2005/06/13 04:35:34  dave
 * ! needs to be not !
 *
 * Revision 1.57  2005/06/13 04:02:06  dave
 * new dryrun feature to keep things from being updated
 *
 * Revision 1.56  2005/06/11 02:32:54  dave
 * simple changes, getting Net for MarketInfo
 *
 * Revision 1.55  2005/06/08 23:28:07  dave
 * ok,  more collateral
 *
 * Revision 1.54  2005/06/08 21:28:50  dave
 * Collateral Common
 *
 * Revision 1.53  2005/06/08 20:31:08  dave
 * more changes to start picking up attributes for collateral
 *
 * Revision 1.52  2005/06/08 19:40:08  dave
 * need new SP
 *
 * Revision 1.51  2005/06/08 19:37:15  dave
 * ok.. lets try to start updating sp information
 *
 * Revision 1.50  2005/06/08 18:05:44  dave
 * CollateralCollection Build all
 *
 * Revision 1.49  2005/06/08 13:21:21  dave
 * testing waters to see if structures make sense outside of WWProductId
 *
 * Revision 1.48  2005/06/07 14:38:36  dave
 * commit and close
 *
 * Revision 1.47  2005/06/07 13:21:24  dave
 * closing the loop and posting time back to the
 * timetable
 *
 * Revision 1.46  2005/06/07 05:28:42  dave
 * more cleanup and NLSid ing
 *
 * Revision 1.45  2005/06/07 04:49:47  dave
 * minor fix to syntax
 *
 * Revision 1.44  2005/06/07 04:44:04  dave
 * secondary fixes to commit chunk
 *
 * Revision 1.43  2005/06/07 04:34:51  dave
 * working on commit control
 *
 * Revision 1.42  2005/06/07 03:54:23  dave
 * not callable fix
 *
 * Revision 1.41  2005/06/07 03:43:28  dave
 * adding the idl piece
 *
 * Revision 1.40  2005/06/07 03:25:29  dave
 * new IDL sp and approach
 *
 * Revision 1.39  2005/06/05 16:54:52  dave
 * rolling in the feature concept
 *
 * Revision 1.38  2005/06/05 00:51:18  dave
 * lets do the put
 *
 * Revision 1.37  2005/06/04 22:22:57  dave
 * ok.. lets try to dump out the ProdStruct XML
 * for now.. lets embed it in the other XML
 *
 * Revision 1.36  2005/06/04 16:46:05  dave
 * more trace
 *
 * Revision 1.35  2005/06/03 21:25:51  dave
 * lets start getting prodstruct data
 *
 * Revision 1.34  2005/06/02 08:16:19  dave
 * ok.. lets give it a whirl
 *
 * Revision 1.33  2005/06/02 04:49:55  dave
 * more clean up
 *
 * Revision 1.32  2005/06/01 06:31:17  dave
 * ok.. lets see if we can do some ProdStruct Damage
 *
 * Revision 1.31  2005/06/01 02:20:11  dave
 * ok.. more cleanup and ve work
 *
 * Revision 1.30  2005/05/29 00:25:30  dave
 * ok.. clean up and reorg to better support pulls from PDH
 *
 * Revision 1.29  2005/05/28 23:25:59  dave
 * ok.. can we spit out xml?
 *
 * Revision 1.28  2005/05/27 22:52:33  dave
 * lets see if we can get something from the PDH
 *
 * Revision 1.27  2005/05/27 22:19:41  dave
 * need to save off list
 *
 * Revision 1.26  2005/05/27 22:06:51  dave
 * trying for a single level dump
 *
 * Revision 1.25  2005/05/27 21:46:59  dave
 * trying to split it up
 *
 * Revision 1.24  2005/05/27 05:13:36  dave
 * ok.. expanding profile information
 *
 * Revision 1.23  2005/05/27 05:09:08  dave
 * adding profile stuff to main test
 *
 * Revision 1.22  2005/05/27 04:58:57  dave
 * fixing CollectionId
 *
 */
