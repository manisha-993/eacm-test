 ...
 *
 * $Log: ProductCollection.java,v $
 * Revision 1.24  2011/05/05 11:21:33  wendy
 * src from IBMCHINA
 *
 * Revision 1.20  2008/11/18 21:16:53  rick
 * LACTO
 *
 * Revision 1.19  2007/11/15 03:16:53  liubdl
 * *** empty log message ***
 *
 * Revision 1.18  2007/11/15 02:59:10  liubdl
 * *** empty log message ***
 *
 * Revision 1.17  2007/11/14 07:14:04  liubdl
 * *** empty log message ***
 *
 * Revision 1.16  2007/11/14 06:52:50  liubdl
 * *** empty log message ***
 *
 * Revision 1.15  2007/10/29 09:32:20  jingb
 * Change code in method syncToCatDb():
 * delete the temp table data by sessionID
 *
 * Revision 1.14  2007/10/19 07:51:40  jiehou
 * *** empty log message ***
 *
 * Revision 1.13  2007/10/19 03:44:00  jiehou
 * *** empty log message ***
 *
 * Revision 1.12  2007/10/19 03:33:41  jiehou
 * *** empty log message ***
 *
 * Revision 1.11  2007/10/19 01:51:04  jiehou
 * *** empty log message ***
 *
 * Revision 1.10  2007/10/18 08:34:19  jiehou
 * *** empty log message ***
 *
 * Revision 1.9  2007/10/18 03:28:46  jiehou
 * *** empty log message ***
 *
 * Revision 1.8  2007/10/16 09:52:05  jiehou
 * *** empty log message ***
 *
 * Revision 1.7  2007/10/16 08:53:44  jiehou
 * *** empty log message ***
 *
 * Revision 1.6  2007/10/16 05:35:09  jingb
 * *** empty log message ***
 *
...
public class ProductCollection
   ...
   * FIELD - LSEO VE NAME
   */
  public static final String LSEO_VE = "VELSEO1";

  public static final String MODEL_VE = "VEMODELPRODUCT1";

  public static final String LSEOBUNDLE_VE = "VELSEOBUNDLE1";
  //Added by Guo Bin, 2007-8-03
  private static String SP_SUFFIX = "E";
   ...
   *
   */
  public ProductCollection(ProductCollectionId _cid) {
    super(_cid);
  }
   ...
   *
   */
  public ProductCollection(Catalog _cat, ProductCollectionId _cid) {
    this(_cid);
    get(_cat);
  }
  ...
  public static void main(String[] args) {
    ...
    if (args[0].equals("1")) {
      ProductCollection.syncToCatDb();
    }
    else if (args[0].equals("2")) {
      ProductCollection.idlToCatDb();
    }
    else {
  ...
  public final String dump(boolean _b) {
    ...
    String strHeader = "**ProductCollection **\n";
    if (wwpid != null) {
  ...
  /**
   ...
   public void deactivateStructure(ProductId _pid, Catalog _cat,boolean _bcommit) {
     ...
     */
    try {
        db.callGBL4025(new ReturnStatus( -1), strEnterprise, strLocEntityType, iLocEntityID, strCountryCode,
  ...
  public void get(Catalog _cat) {
    ...
    try {
      ...
      if (pcid.isFromCAT()) {
        D.ebug(D.EBUG_SPEW,"ProductCollection: isFromCat()");
        if (pcid.getCollectionType() ==
            ProductCollectionId.BY_WWENTITYTYPE_WWENTITYID) {
          D.ebug(D.EBUG_SPEW,"ProductCollection: pcid.getCollectionType() == ProductCollectionId.BY_WWENTITYTYPE_WWENTITYID");
          try {
...
//    }
    ...
    try {
      ...
      prof.setValOnEffOn(cati.getEndDate(), cati.getEndDate());
		...
		int isLimitCountryCode=CatalogProperties.getLimitCountryCode(ProductCollection.class,Product.class);
		if (db.getPDHConnection() == null) {
			db.connect();
		}	
		D.ebug(D.EBUG_SPEW, "ProductCollection.Product.LIMITCOUNTRYCODE is : "+isLimitCountryCode);
      ...
      for (int icycle = 0; icycle < 3; icycle++) {
        ...
        if (smcOuter != null) {
          ...
          if(icycle == 2) {
		      ...
		      strVE = ProductCollection.LSEOBUNDLE_VE;
              ...
              D.ebug(D.EBUG_SPEW,"D");
				...
				prod.get(cat);
      		    ...
      		    D.ebug(D.EBUG_SPEW,"syncToCatDb() .. updating the prod to the CatDb... " +
      		           prod.toString());
		      if (isLimitCountryCode==0 || (isLimitCountryCode==1 && !hasCountryCode(db.getPDHConnection(),gami.getEnterprise(), gami.getCountry(), prod))) {
		    	  prod.put(cat, false);    
			  }      		    
		      if (icount == ichunk) {
      		      db.commit();
...
//    }
    ...
    try {
      ...
      for (int icycle = 0; icycle < 3; icycle++) {
        ...
        iNLSSave[0] = CatalogProperties.getSaveNLSID(ProductCollection.class,Product.class, strVE);
        iNLSSave[1] = CatalogProperties.getSaveNLSID(ProductCollection.class,ProdStruct.class, strVE);
        D.ebug(D.EBUG_SPEW,"ProductCollection.Product."+strVE+"_nlssave="+iNLSSave[0]);
        D.ebug(D.EBUG_SPEW,"ProductCollection.ProdStruct."+strVE+"_nlssave="+iNLSSave[1]);
        //
        // Newest version of chunking
        //
        int iChunk = 1000;
    ...
    while (it.hasNext()) {
       ...
       SMI_LOOP:
     	 for (int x = 0; x < prod.getSmc().getCount(); x++) {
             ...
             if(smi.getChildTran().equals("ON") /* && prod.matchesCATLGPUB(_cat) */ ) {
				 prod.setActive(true);
				 break SMI_LOOP;
			 }
		 }
  ...
  private static boolean hasCountryCode(Connection conn,String enterprise, String countrycode,Product product) {
		boolean isExist = false;
		PreparedStatement stmt=null;
		String sql="select COUNTRYCODE from GBLI.Product where COUNTRYCODE=? and ENTERPRISE=? and LOCENTITYTYPE=? and LOCENTITYID=? and isactive=1 fetch first 1 rows only";
		try{
			stmt=conn.prepareStatement(sql);
			stmt.setString(1,countrycode);
			stmt.setString(2,enterprise);
			stmt.setString(3,product.getProductId().getEntityType());
			//stmt.setInt(4,product.getProductId().getEntityId());
			stmt.setInt(4,product.getIntVal(Product.LOCENTITYID));
