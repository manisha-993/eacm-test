...
db2 "update eacm.ifmlock set status=0 where process_id in (1,50)"
db2 "delete from eacm.collectionstatus"
db2 "import from cs_compat.del of del insert into eacm.collectionstatus"
...
sleep 30
db2 "update eacm.ifmlock set status=0 where process_id in (1,50)"
echo "ifmlock status reset successfully"
db2 terminate
