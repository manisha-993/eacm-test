...
//
// $Log: ALWRCSOLABR001PDG.java,v $
// Revision 1.23  2008/09/04 20:31:48  wendy
// Cleanup RSA warnings
//
// Revision 1.22  2006/02/14 16:59:11  joan
// some adjustments
//
// Revision 1.21  2005/04/28 21:15:36  joan
// fixes
//
// Revision 1.20  2003/12/05 00:26:37  joan
// add CD
//
// Revision 1.19  2003/11/25 20:34:04  joan
// adjust copy
//
// Revision 1.18  2003/11/13 17:32:24  joan
// work on CR
//
// Revision 1.17  2003/11/12 23:23:06  joan
// work on CR
//
// Revision 1.16  2003/11/12 20:05:32  joan
// work on CR
//
// Revision 1.15  2003/10/20 17:42:33  joan
// fb fixes
//
// Revision 1.14  2003/10/14 19:57:18  joan
// work on fb
//
// Revision 1.13  2003/10/13 21:28:59  joan
// fix the report
//
// Revision 1.12  2003/10/06 23:34:21  joan
// work on LS ABR
//
// Revision 1.11  2003/09/15 21:39:29  joan
// change to Extract GA
//
// Revision 1.10  2003/09/04 21:55:18  joan
// fix fb
//
// Revision 1.9  2003/08/13 22:43:30  joan
// fix report
//
// Revision 1.8  2003/08/04 22:31:29  joan
// fix bug
//
// Revision 1.7  2003/08/01 20:21:27  joan
// fix typo USERTOKEN
//
// Revision 1.6  2003/07/31 00:11:41  joan
// fix bug
//
// Revision 1.5  2003/07/30 23:35:46  joan
// fix bug
//
// Revision 1.3  2003/07/30 22:06:11  joan
// adjust code
//
// Revision 1.2  2003/07/30 21:05:39  joan
// add some code for DG rpt name
//
// Revision 1.1  2003/07/28 21:20:06  joan
// add ALWRECSOLABR001
//
...
public class ALWRCSOLABR001PDG extends PDGActionItem {
	...
	private EntityItem m_eiCDCurrent = null;
  	...
  	*/
  	public String getVersion() {
  		return new String("$Id: ALWRCSOLABR001PDG.java,v 1.23 2008/09/04 20:31:48 wendy Exp $");
  	}


	public ALWRCSOLABR001PDG(EANMetaFoundation  _mf, ALWRCSOLABR001PDG _ai) throws MiddlewareRequestException {
		super(_mf, _ai);
	}
	...
	*/
	public ALWRCSOLABR001PDG(EANMetaFoundation _emf, Database _db, Profile _prof, String _strActionItemKey) throws SQLException, MiddlewareException, MiddlewareRequestException {
	  	super(_emf, _db,  _prof, _strActionItemKey);
	}

  	public String dump(boolean _bBrief) {
   		StringBuffer strbResult = new StringBuffer();
   		strbResult.append("ALWRCSOLABR001PDG:" + getKey() + ":desc:" + getLongDescription());
	...
	protected StringBuffer checkMissingData(Database _db, Profile _prof, boolean _bGenData) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		//String strTraceBase = " ALWRCSOLABR001PDG executeProduct method";
		StringBuffer sbReturn = new StringBuffer();

		StringBuffer sb = new StringBuffer();

		String strFileName = "PDGtemplates/PCDPDG2.txt";
		...
		for (int i=0; i < m_cdList.size(); i++) {
			...
			if (attF != null) {
				...
				for (int x=0; x < amf.length; x++) {
					...
					if (mf.isSelected()) {
						...
						if (att != null) {
							...
							infoList.put("CSOL_T1", s.substring(0,5));
							strCSOL_T1 = s.substring(0,5);
						}
						...
						if (_bGenData) {
							if (sbMissing.toString().length() > 0) {
								...
								if (m_sbActivities.toString().length() > 0) {
									...
									sb.append("<br /><LI>CSOL already exists for Part Number =" + strCSOL_T1 + strCD_T1 + " and General Area = " + mf.getLongDescription());
								}
							} else {
								sb.append("<br /><LI>CSOL already exists for Part Number =" + strCSOL_T1 + strCD_T1 + " and General Area = " + mf.getLongDescription());
							}
				...
				if (!bPopulated) {
					sb.append("<br /><LI>No CSOL generated since the CD's GENAREANAMECTYF not populated.");
				}
			} else {
				sb.append("<br /><LI>No CSOL generated since the CD's GENAREANAMECTYF not populated.");
			}
	...
	public byte[] viewMissing(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		//String strTraceBase = " ALWRCSOLABR001PDG viewMissingEntities method";
		return null;
	}
	...
	public void executeAction(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " ALWRCSOLABR001PDG executeAction method ";
		m_SBREx = new SBRException();
		try {
			_db.debug(D.EBUG_DETAIL, strTraceBase);
			if (m_eiPDG == null) {
				System.out.println("CSOL entity is null");
				return;
			}
