...
//
// $Log: SWSUBSFEATINI30APDG.java,v $
// Revision 1.14  2008/09/05 21:25:46  wendy
// Cleanup RSA warnings
//
// Revision 1.13  2007/09/06 12:39:01  couto
...
// fixes
//
// Revision 1.4  2004/09/03 15:58:01  joan
// fixes
//
...
public class SWSUBSFEATINI30APDG extends PDGActionItem {
  ...
  static final long serialVersionUID = 20011106L;
	...
	//private EANList m_opList = new EANList();
  	...
  	*/
  	public String getVersion() {
  		return new String("$Id: SWSUBSFEATINI30APDG.java,v 1.14 2008/09/05 21:25:46 wendy Exp $");
  	}


	public SWSUBSFEATINI30APDG(EANMetaFoundation  _mf, SWSUBSFEATINI30APDG _ai) throws MiddlewareRequestException {
		super(_mf, _ai);
	}

	public SWSUBSFEATINI30APDG(EANMetaFoundation _emf, Database _db, Profile _prof, String _strActionItemKey) throws SQLException, MiddlewareException, MiddlewareRequestException {
	  	super(_emf, _db,  _prof, _strActionItemKey);
	}

  	public String dump(boolean _bBrief) {
   		StringBuffer strbResult = new StringBuffer();
   		strbResult.append("SWSUBSFEATINI30APDG:" + getKey() + ":desc:" + getLongDescription());
	...
	protected StringBuffer checkMissingData(Database _db, Profile _prof, boolean _bGenData) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		//String strTraceBase = " SWSUBSFEATINI30APDG checkMissingData method";
		...
		for(int i=0; i < m_afRegTypeVec.size(); i++) {
			...
			String strFileName = "PDGtemplates/SubsFeatInitial_30a.txt";
			if (strRegType.equals("Grp2Grp")) {			// Grp2Grp
				if (! m_strAfBillingTemplate.equals("NOBILLING")) {
					strFileName = "PDGtemplates/SubsFeatInitial_2_30a.txt";
				}
				...
				if (m_strAfBillingTemplate.equals("IASP003") || m_strAfBillingTemplate.equals("IASP108V2") || m_strAfBillingTemplate.equals("IASP141")) {
					strFileName = "PDGtemplates/SubsFeatInitial_1_30a.txt";
				} else if (m_strAfBillingTemplate.equals("IASP096") || m_strAfBillingTemplate.equals("IASP098")) {
					strFileName = "PDGtemplates/SubsFeatInitial_3_30a.txt";
				} else if (m_strAfBillingTemplate.equals("IASP147")) {
					strFileName = "PDGtemplates/SubsFeatInitial_4_30a.txt";
				}
			}
	...
	//	String strTraceBase = " SWSUBSFEATINI30APDG checkPDGAttribute method";
		...
		if ( !"Subscription Feature".equals(m_strAfReqType)) {
	...
	public byte[] viewMissing(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " SWSUBSFEATINI30APDG viewMissing method";
	...
	public void executeAction(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " SWSUBSFEATINI30APDG executeAction method";
		//String strData = "";
		try {
			_db.debug(D.EBUG_DETAIL, strTraceBase);
			m_SBREx = new SBRException();
 ...
 */
	...
	protected void checkDataAvailability(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		...
		if (aeiAPPCOM.length <= 0 ) {
			m_SBREx.add("The MODEL with classifications SW-Subscription-Base-N/A, MACHTYPE=" + m_strAfMachType + ", MODEL=" + m_strAfModel);
		} else if (aeiAPPCOM.length > 1) {
			m_SBREx.add("There are " + aeiAPPCOM.length + " existing MODELs with classifications SW-Subscription-Base, MACHTYPE=" + m_strAfMachType + ", MODEL=" + m_strAfModel+ ", OPTFEATUREID=" + m_strAfOptFeature);
		} else {
		...
		if (aeiCOM.length <= 0 ) {
			m_SBREx.add("The MODEL with classifications SW-SubscrFeature-Base, MACHTYPE=" + m_strAfMachType + ", MODEL=" + m_strAfModel+ ", OPTFEATUREID=" + m_strAfOptFeature + " must be created before Initial MODEL.");
		} else if (aeiCOM.length > 1) {
			m_SBREx.add("There are " + aeiCOM.length + " existing MODELs with classifications SW-SubscrFeature-Base, MACHTYPE=" + m_strAfMachType + ", MODEL=" + m_strAfModel+ ", OPTFEATUREID=" + m_strAfOptFeature);
		} else {
