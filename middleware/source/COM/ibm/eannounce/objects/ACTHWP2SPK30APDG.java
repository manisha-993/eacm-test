...
//
// $Log: ACTHWP2SPK30APDG.java,v $
// Revision 1.15  2008/09/04 19:49:49  wendy
// Cleanup RSA warnings
//
// Revision 1.14  2007/09/14 18:32:07  couto
// MN32841099 WGMODEL replaced by WGMODELA
//
// Revision 1.13  2006/02/20 21:39:44  joan
// clean up System.out.println
//
// Revision 1.12  2005/02/28 20:29:50  joan
// add throw exception
//
...
//
// Revision 1.9  2004/11/15 17:00:42  joan
// fix compile
//
// Revision 1.8  2004/11/12 23:50:32  bala
// dont check for MODELCONVERT if the from and to models are the same
//
// Revision 1.7  2004/10/10 00:53:25  bala
// fix
//
// Revision 1.6  2004/10/06 19:37:03  bala
// fixes
//
// Revision 1.5  2004/10/05 22:51:45  bala
// fix
//
// Revision 1.4  2004/10/05 22:45:20  bala
// Add the MODELCONVERT check
//
// Revision 1.3  2004/09/29 17:24:13  bala
// wrong abr launched
//
// Revision 1.2  2004/09/28 22:04:34  bala
// use another extract
//
// Revision 1.1  2004/09/23 22:04:01  bala
// check in
//
 ...
 */
public class ACTHWP2SPK30APDG extends PDGActionItem {
   ...
   */
  /**
   *  Gets the version attribute of the ACTHWP2SPK30APDG object
   *
   *@return    The version value
   */
  public String getVersion() {
    return new String("$Id: ACTHWP2SPK30APDG.java,v 1.15 2008/09/04 19:49:49 wendy Exp $");
  }


  /**
   *  Constructor for the ACTHWP2SPK30APDG object
   *
   ...
   */
  public ACTHWP2SPK30APDG(EANMetaFoundation _mf, ACTHWP2SPK30APDG _ai) throws MiddlewareRequestException {
    super(_mf, _ai);
    m_bCollectInfo = true;
    m_iCollectStep = 1;
  }
   ...
   *@exception  MiddlewareRequestException  Description of the Exception
   */
  public ACTHWP2SPK30APDG(EANMetaFoundation _emf, Database _db, Profile _prof, String _strActionItemKey) throws SQLException, MiddlewareException, MiddlewareRequestException {
  ...
  public String dump(boolean _bBrief) {
    ...
    strbResult.append("ACTHWP2SPK30APDG:" + getKey() + ":desc:" + getLongDescription());
  ...
  /**
   *  Gets the purpose attribute of the ACTHWP2SPK30APDG object
   *
   *@return    The purpose value
   */
  public String getPurpose() {
    return "ACTHWP2SPK30APDG";
  }


  /**
   *  Gets the stepDescription attribute of the ACTHWP2SPK30APDG object
   *
  ...
  /**
   *  Sets the pDGCollectInfo attribute of the ACTHWP2SPK30APDG object
   *
 ...
 */
  ...
  protected StringBuffer checkMissingData(Database _db, Profile _prof, boolean _bGenData) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
    String strTraceBase = " ACTHWP2SPK30APDG checkMissingData method";
    //boolean bProc2Proc = false;
    boolean bProc2ServerPkg = false;
    boolean bProcICard2ServerPkg = false;
    ...
    for (int i = 0; i < m_upgradeVec.size(); i++) {
      ...
      if (iIcards == 2) {// Its a Processor to I card Server Pkg
        bProc2ServerPkg = true;
        // Upgrade from PROC to SERVER|PACKAGE
        strFileName = "PDGtemplates/HWUpgrade30aProc1toServer2_Pack1.txt";
        if (m_strAfSerType.equals("iSeries")) {
          infoList.put("PROCUPGTYPE", "160");
        }
      } else {//Its a Processor Icard to  Icard SErver Pkg
        bProcICard2ServerPkg = true;
        // Upgrade from PROC|ICARD to SERVER|PACKAGE
        strFileName = "PDGtemplates/HWUpgrade30aProc1_Icard1toServer2_Pack2.txt";
        infoList.put("PROCUPGTYPE", "150");
      }
      ...
      while (st.hasMoreTokens()) {
        ...
        if (iCount == 0) {//We are at the From part of the String
          if (bProcICard2ServerPkg) {//If true check for Icard separator "|"
          ...
          if (bProcICard2ServerPkg || bProc2ServerPkg) {//If true check for Icard separator "|"
            st1 = new StringTokenizer(strToken, "|");
            strToServer = st1.nextToken();
            strToPkg = st1.nextToken();
            m_eiToServer = (EntityItem) m_eiList.get(strToServer + "TOSERV");
            _db.debug(D.EBUG_DETAIL, "checkMissingData:strToServer:" + strToServer);
            m_eiToPkg = (EntityItem) m_eiList.get(strToPkg + "TOPKG");
            _db.debug(D.EBUG_DETAIL, "checkMissingData:strToPkg:" + strToPkg);
          } else {
            strToServer = strToken;
            m_eiToServer = (EntityItem) m_eiList.get(strToServer + "TOSERV");
            _db.debug(D.EBUG_DETAIL, "checkMissingData:strToServer:" + strToServer);
          }
        }
...
//					m_sbActivities.append("<TEMPLATEFILE>" + strFileName + "</TEMPLATEFILE>\n");
  ...
  protected void checkPDGAttribute(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareRequestException, MiddlewareException, SBRException, MiddlewareShutdownInProgressException {
    String strTraceBase = " ACTHWP2SPK30APDG checkPDGAttribute method";
  ...
  public void queuedABR(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
    String strTraceBase = "ACTHWP2SPK30APDG queuedABR method";
    _db.debug(D.EBUG_DETAIL, strTraceBase);
    ...
    attL.put("HWP2SPK30APDGABR01", "HWP2SPK30APDGABR01=0020");
    _prof = m_utility.setProfValOnEffOn(_db, _prof);
  ...
  public byte[] viewMissing(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
    String strTraceBase = " ACTHWP2SPK30APDG viewMissing method";
  ...
  public void executeAction(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
    String strTraceBase = " ACTHWP2SPK30APDG executeAction method";
    String strData = "";
    try {
      _db.debug(D.EBUG_DETAIL, strTraceBase);
      m_SBREx = new SBRException();
      ...
      m_sbActivities.append("<PDGACTTIONITEM>" + getLongDescription() + "</PDGACTTIONITEM>");
      m_sbData = new StringBuffer();
      //m_xai = new ExtractActionItem(null, _db, _prof, "EXTHWP2SPK30APDG");
    ...
    for (int i = 0; i < m_upgradeVec.size(); i++) {
     // bProc2Proc = false;
      ...
      while (st.hasMoreTokens()) {
        ...
        if (iCount == 0) {//We are at the From part of the String
          ...
          if (bProcICard2ServerPkg || bProc2ServerPkg) {//If true check for Icard separator "|"
            ...
            strToServer = st1.nextToken();
            strToPkg = st1.nextToken();
            _db.debug(D.EBUG_DETAIL, "checkDataAvailability:strToServer:" + strToServer);
      ...
      if (strToServer!=null) {
        ...
        sb1.append("map_FEATURE:FEATURECODE:P=" + strToServer + ";");
        sb1.append("map_MODEL:MACHTYPEATR:C=" + m_strToMachType + ";");
        sb1.append("map_MODEL:MODELATR:C=" + m_strToModel);

        EntityItem[] aeiUpgrade = m_utility.dynaSearchII(_db, _prof, m_eiPDG, strSai1, "PRODSTRUCT", sb1.toString());
        if (aeiUpgrade == null || aeiUpgrade.length <= 0) {
          m_SBREx.add(" UpgradeTo PRODSTRUCT for " + strToServer + " not found.");
        } else {
          _db.debug(D.EBUG_DETAIL, "checkDataAvailability:PRODSTRUCT found for strToServer:" + strToServer + ":" + aeiUpgrade[0].getKey());
          m_eiList.put(strUpgrade, aeiUpgrade[0]);
          //Store the FEATURE ENTITY for later use
          ...
          m_eiList.put(strToServer + "TOSERV", m_eiToServer);
        }
      ...
      if (strToPkg!=null) {
        ...
        if (aeiUpgrade == null || aeiUpgrade.length <= 0) {
          ...
          _db.debug(D.EBUG_DETAIL, "checkDataAvailability:TOICARD found for :" + m_eiToPkg.getKey());
