...
//
// $Log: HWFUPPOFPDG.java,v $
// Revision 1.10  2008/09/04 21:21:25  wendy
// Cleanup RSA warnings
//
// Revision 1.9  2004/01/14 23:28:28  joan
// fix bug
//
// Revision 1.8  2004/01/14 21:02:27  joan
// fix bug
//
// Revision 1.7  2004/01/14 19:47:45  joan
// fix null pointer
//
// Revision 1.6  2003/12/16 23:40:49  joan
// work on CR
//
// Revision 1.5  2003/12/09 17:18:20  joan
// fix code
//
// Revision 1.4  2003/12/08 22:41:43  joan
// make isDigit public
//
// Revision 1.3  2003/12/03 23:07:48  joan
// adjust code
//
// Revision 1.2  2003/12/03 18:09:06  joan
// add HWFUPPOFPDG
//
// Revision 1.1  2003/12/02 00:20:43  joan
// initial load
//
...
public class HWFUPPOFPDG extends PDGActionItem {
	static final long serialVersionUID = 20011106L;
	private StringBuffer m_sbData = new StringBuffer();
	private String m_strAfFeatureCode = null;
	private String m_strAfSeries = null;

  	/*
  	* Version info
  	*/
  	public String getVersion() {
  		return new String("$Id: HWFUPPOFPDG.java,v 1.10 2008/09/04 21:21:25 wendy Exp $");
  	}


	public HWFUPPOFPDG(EANMetaFoundation  _mf, HWFUPPOFPDG _ai) throws MiddlewareRequestException {
		super(_mf, _ai);
	}

	public HWFUPPOFPDG(EANMetaFoundation _emf, Database _db, Profile _prof, String _strActionItemKey) throws SQLException, MiddlewareException, MiddlewareRequestException {
	  	super(_emf, _db,  _prof, _strActionItemKey);
	}

  	public String dump(boolean _bBrief) {
   		StringBuffer strbResult = new StringBuffer();
   		strbResult.append("HWFUPPOFPDG:" + getKey() + ":desc:" + getLongDescription());
	...
	//	String strTraceBase = " HWFUPPOFPDG checkMissingData method";

	//	EntityItem eiFUP = (EntityItem)m_eiList.get("FUP");

		String strFileName = "PDGtemplates/HWFUPPOF.txt";
	...
	protected void checkPDGAttribute(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareRequestException, MiddlewareException, SBRException, MiddlewareShutdownInProgressException {
		//String strTraceBase = " HWFUPPOFPDG checkPDGAttribute method";
		for (int i =0; i < _afirmEI.getAttributeCount(); i++) {
			...
			if (att.getKey().equals("SERIES")) {
				if (mFlagAtt.size() > 1) {
					m_SBREx.add("Please select only one Series");
				} else if (mFlagAtt.size() == 1) {
					m_strAfSeries = (String)mFlagAtt.elementAt(0);
				}
			} else if (att.getKey().equals("FEATURECODE")) {
				m_strAfFeatureCode = textAtt;
			}
		}

		if (m_strAfFeatureCode != null ) {
			if (m_strAfFeatureCode.length() != 4) {
				m_SBREx.add("Feature Code length has to be 4.");
			}

			if (!m_utility.isDigit(m_strAfFeatureCode)) {
				m_SBREx.add("Feature Code has to be numeric.");
			}
		}
	}

	protected void resetVariables() {
		m_strAfFeatureCode = null;
		m_strAfSeries = null;
	}
	...
	public byte[] viewMissing(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " HWFUPPOFPDG executeAction method";
		...
		_prof = m_utility.setProfValOnEffOn(_db, _prof);
		ExtractActionItem eaItem = new ExtractActionItem(null, _db, _prof, "EXTAFIRMT8");
		...
		_db.test(m_eiRoot != null, "Offering Project entity is null");
 ...
 */
	...
	protected void checkDataAvailability(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		// make sure the FUP already exists
		if (m_strAfFeatureCode != null && m_strAfFeatureCode.length() > 0) {
			String strSai = (String)m_saiList.get("FUP");
			StringBuffer sb = new StringBuffer();
			sb.append("map_SERIES=" + m_strAfSeries + ";");
			sb.append("map_FEATURECODE=" + m_strAfFeatureCode);

			EntityItem[] aeiFUP = m_utility.dynaSearch(_db, _prof, m_eiPDG, strSai, "FUP", sb.toString());

			if (aeiFUP.length > 0) {
				m_SBREx.add("There are " + aeiFUP.length + "  existing Function Point for " + m_strAfSeries + "-" + m_strAfFeatureCode);
				for (int i = 0; i < aeiFUP.length; i++) {
					EntityItem ei = (EntityItem) aeiFUP[i];
					m_SBREx.add("\n" + ei.toString());
				}
			}
		}
