...
//
// $Log: HWUPGRADEPSPDG.java,v $
// Revision 1.5  2008/09/04 21:46:54  wendy
// Cleanup RSA warnings
//
// Revision 1.4  2005/02/28 20:29:51  joan
// add throw exception
//
// Revision 1.3  2004/04/20 19:52:54  joan
// work on duplicate
//
...
public class HWUPGRADEPSPDG extends PDGActionItem {
  ...
  static final long serialVersionUID = 20011106L;
	...
	//private OPICMList m_checkedSearchList = new OPICMList();
  	...
  	*/
  	public String getVersion() {
  		return new String("$Id: HWUPGRADEPSPDG.java,v 1.5 2008/09/04 21:46:54 wendy Exp $");
  	}


	public HWUPGRADEPSPDG(EANMetaFoundation  _mf, HWUPGRADEPSPDG _ai) throws MiddlewareRequestException {
		super(_mf, _ai);
		m_bCollectInfo = true;
		m_iCollectStep = 1;
	}

	public HWUPGRADEPSPDG(EANMetaFoundation _emf, Database _db, Profile _prof, String _strActionItemKey) throws SQLException, MiddlewareException, MiddlewareRequestException {
  	...
  	public String dump(boolean _bBrief) {
   		...
   		strbResult.append("HWUPGRADEPSPDG:" + getKey() + ":desc:" + getLongDescription());
	...
	protected void generateData(Database _db, Profile _prof, StringBuffer _sbMissing, String _strSearchKey) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		//String strTraceBase = "HWUPGRADEPSPDG generateData";
		try {
	...
	//	String strTraceBase = " HWUPGRADEPSPDG checkMissingData method";
			...
			if (m_processedComboList.get(strFrom + strTo) == null) {
				...
				while (st1.hasMoreTokens()) {
					...
					if (c==0) {
						...
						_db.test(m_eiFromICard != null, "From ICard Entity is null");
				...
				while (st1.hasMoreTokens()) {
					...
					if (c==0) {
						strToServ = s;
						m_eiToServ = (EntityItem)m_eiList.get(s+"FUP");
					} else if (c==1) {
						strToPack = s;
						if (strFrom.indexOf("|") >= 0) {
							m_eiToPack = (EntityItem)m_eiList.get(s+"FUP");
						} else {
							m_eiToPack = (EntityItem)m_eiList.get(s+"OOF");
						}
					}
					c++;
				}
				...
				if (strFromProc != null && strToServ != null && strFromICard == null && strToPack != null) {
					// Upgrade from PROC to SERVER|PACKAGE
					strFileName = "PDGtemplates/HWUpgradeProc1toServer2_Pack1.txt";
				} else if (strFromProc != null && strToServ != null && strFromICard != null && strToPack != null) {
					// Upgrade from PROC|ICARD to SERVER|PACKAGE
						strFileName = "PDGtemplates/HWUpgradeProc1_Icard1toServer2_Pack2.txt";
				}
...
//					m_sbActivities.append("<TEMPLATEFILE>" + strFileName + "</TEMPLATEFILE>\n");
					...
					_db.test(m_eiFromProc != null, "From Processor Entity is null");
		//			System.out.println(strTraceBase + " m_eiFromProc " + m_eiFromProc.dump(false));
	...
	protected void checkPDGAttribute(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareRequestException, MiddlewareException, SBRException, MiddlewareShutdownInProgressException {
		//String strTraceBase = " HWUPGRADEPSPDG checkPDGAttribute method";
	...
	public byte[] viewMissing(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " HWUPGRADEPSPDG executeAction method";
	...
	public void executeAction(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		...
		try {
			...
			m_sbActivities.append("<PDGACTTIONITEM>" + getLongDescription() + "</PDGACTTIONITEM>");
			m_sbData = new StringBuffer();
//			m_PDGxai = new ExtractActionItem(null, _db, _prof, "EXTSWPDG1");
			...
			if (strFrom != null) {
				...
				while (st.hasMoreTokens()) {
					...
					if (m_eiList.get(s+"FUP") == null) {
						...
						String strFUP = "Hardware FUP ";
						if (count == 0) {
							sb.append("map_FUPSUBCAT=604;");
							sb.append("map_FUPGRP=400;");
							strFUP = "Hardware-Processor-iSeries FUP ";
							//strFromProc = s;
						} else {
							//strFromIcard = s;
						}
						...
						strFUP = strFUP + "for SERIES=iSeries and featurecode " + s;
						...
						if (m_utility.findEntityItem(m_eiList, "FUP", sb.toString()) == null) {
							...
							if (aeiFUP.length <= 0 ) {
								m_SBREx.add(strFUP + " must exist.");
							} else if (aeiFUP.length > 1) {
								m_SBREx.add("There are " + aeiFUP.length + " existing " + strFUP);
							} else {
				...
				while (st.hasMoreTokens()) {
					...
					if(count == 0) {
						...
						if (m_eiList.get(s+"FUP") == null) {
							...
							sb.append("map_SERIES=6129;");
							sb.append("map_FEATURECODE=" + s );
							String strFUP = "Hardware-Server-iSeries FUP for SERIES=iSeries and featurecode " + s;
							if (m_utility.findEntityItem(m_eiList, "FUP", sb.toString()) == null) {
								EntityItem[] aeiFUP = m_utility.dynaSearch(_db, _prof, m_eiPDG, strSai, "FUP", sb.toString());
								if (aeiFUP.length <= 0 ) {
									m_SBREx.add(strFUP + " must exist.");
						...
						if (strFrom.indexOf("|") > -1) {
							// upgrade Proc|Icard to Server|Package, need to look for FUP
							if (m_eiList.get(s+"FUP") == null) {
								strSai = (String)m_saiList.get("FUP");
								sb = new StringBuffer();
								sb.append("map_FUPCAT=100;");
								...
								sb.append("map_SERIES=6129;");
								sb.append("map_FEATURECODE=" + s );
								String strFUP = "Hardware-Package-iSeries FUP for SERIES=iSeries and featurecode " + s;
								if (m_utility.findEntityItem(m_eiList, "FUP", sb.toString()) == null) {
									EntityItem[] aeiFUP = m_utility.dynaSearch(_db, _prof, m_eiPDG, strSai, "FUP", sb.toString());
									if (aeiFUP.length <= 0 ) {
										m_SBREx.add(strFUP + " must exist.");
							...
							//upgrade Proc to Server|Package, need to look for ORDEROF
							if (m_eiList.get(s+"OOF") == null) {
								...
								sb.append("map_MODEL=" + m_strAfModel + ";");
								sb.append("map_FEATURECODE=" + s );
								String strOOF = "Hardware-FeatureCode ORDEROF for " + m_strAfMachType + "-" + m_strAfModel + " and featurecode " + s;
								if (m_utility.findEntityItem(m_eiList, "ORDEROF", sb.toString()) == null) {
									...
									if (aeiOOF.length <= 0 ) {
										m_SBREx.add(strOOF + " must exist.");
									} else if (aeiOOF.length > 1) {
										m_SBREx.add("There are " + aeiOOF.length + " existing " + strOOF);
									} else {
