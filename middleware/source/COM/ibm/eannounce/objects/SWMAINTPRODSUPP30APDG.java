...
//
// $Log: SWMAINTPRODSUPP30APDG.java,v $
...
// fixes
//
// Revision 1.4  2004/09/22 20:25:16  joan
// fixes
//
// Revision 1.3  2004/09/03 15:58:01  joan
// fixes
//
// Revision 1.2  2004/09/02 16:16:47  joan
// add files
//
...
public class SWMAINTPRODSUPP30APDG extends PDGActionItem {
  ...
  static final long serialVersionUID = 20011106L;
	...
	//private EANList m_opList = new EANList();
  	...
  	*/
  	public String getVersion() {
  		return new String("$Id: SWMAINTPRODSUPP30APDG.java,v 1.10 2008/09/05 21:25:47 wendy Exp $");
  	}


	public SWMAINTPRODSUPP30APDG(EANMetaFoundation  _mf, SWMAINTPRODSUPP30APDG _ai) throws MiddlewareRequestException {
		super(_mf, _ai);
	}

	public SWMAINTPRODSUPP30APDG(EANMetaFoundation _emf, Database _db, Profile _prof, String _strActionItemKey) throws SQLException, MiddlewareException, MiddlewareRequestException {
	  	super(_emf, _db,  _prof, _strActionItemKey);
	}

  	public String dump(boolean _bBrief) {
   		StringBuffer strbResult = new StringBuffer();
   		strbResult.append("SWMAINTPRODSUPP30APDG:" + getKey() + ":desc:" + getLongDescription());
	...
	//	String strTraceBase = " SWMAINTPRODSUPP30APDG checkMissingData method";
		StringBuffer sbReturn = new StringBuffer();
		EntityItem eiBaseCOF =(EntityItem)m_eiList.get("BASE");
		_db.test(eiBaseCOF != null, " Base MODEL is null");
		String strFileName = "PDGtemplates/MaintProdSupp_30a.txt";
	...
	protected void checkPDGAttribute(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareRequestException, MiddlewareException, SBRException, MiddlewareShutdownInProgressException {
		//String strTraceBase = " SWMAINTPRODSUPP30APDG checkPDGAttribute method";
	...
	public byte[] viewMissing(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " SWMAINTPRODSUPP30APDG viewMissing method";
	...
	public void executeAction(Database _db, Profile _prof) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		String strTraceBase = " SWMAINTPRODSUPP30APDG executeAction method";
		//String strData = "";
		try {
			_db.debug(D.EBUG_DETAIL, strTraceBase);
			m_SBREx = new SBRException();
 ...
 */
	...
	protected void checkDataAvailability(Database _db, Profile _prof, EntityItem _afirmEI) throws SQLException, MiddlewareException, MiddlewareShutdownInProgressException, SBRException {
		...
		if (aeiCOM.length <= 0 ) {
			m_SBREx.add("The MODEL with classifications SW-Maintenance-Base, MACHTYPE=" + m_strAfMachType + ", MODEL=" + m_strAfModel+ ", OPTFEATUREID=" + m_strAfOptFeature + " must be created before connecting to supported MODEL.");
		} else if (aeiCOM.length > 1) {
			m_SBREx.add(aeiCOM.length + " existing Base MODEL for the same machinetype and model.");
		} else {
		...
		for(int i=0; i < m_afSuppProdVec.size(); i++) {
			...
			if (aeiCOF == null || aeiCOF.length <= 0) {
				m_SBREx.add(" Supported MODELs with classifications Software-Application-Base-N/A, MACHTYPE=" + strMachType + ", MODELATR=" + strModel + " not found.");
			} else {
