...
class XMLEditorKit extends HTMLEditorKit implements XMLEditorGlobals
{
    ...
    private static Hashtable<String, Parser> parserHash = new Hashtable<String, Parser>();  // one parser delegator per DTD type
...
*/
    private Hashtable<String, StyledTextAction> allDtdActions = new Hashtable<String, StyledTextAction>();
    ...
    static final Hashtable<String, String> ACTION_NAMES_TBL;
    private static final Vector<String> ATTR_ACTION_NAMES_VCT, DEP_ACTION_NAMES_VCT;
    static {
        // this is used to match tag to action(s) that is used to create the tag in the editor
        ACTION_NAMES_TBL = new Hashtable<String, String>();
        ATTR_ACTION_NAMES_VCT = new Vector<String>();
        DEP_ACTION_NAMES_VCT = new Vector<String>();
...
//  should any part of listitem be selected?  should entire list item be used?
...
//                     "  p.italicBold {font-style : italic;  font-weight : bold; }");
	...
	Hashtable<String, HashSet<?>> getDTDActions()
    {
        Hashtable<String, HashSet<?>> dtdTbl = new Hashtable<String, HashSet<?>>();
        // get list of action names supported by this dtd
        ...
        for (int i=0; i<keys.length; i++)
        {
            ...
            {
                Set<Action> names = (Set<Action>)dtdTbl.get(DEPENDENT_DTD);
                names.add(a);
            }
            else
            if (ATTR_ACTION_NAMES_VCT.contains(keys[i]))
            {
                Set<Action> names = (Set<Action>)dtdTbl.get(ATTRIBUTE_DTD);
                names.add(a);
            }
            ...
            {
                Set<Action> names =  (Set<Action>) dtdTbl.get(BASIC_DTD);
                names.add(a);
            }
        }
    ...
    {
        Action array[];
        Vector<StyledTextAction> dtdActionsVct = new Vector<StyledTextAction>(1);
        // the editor control is getting instantiated
        // return all possible actions
        for (Enumeration e = allDtdActions.elements(); e.hasMoreElements();) {
            dtdActionsVct.addElement((StyledTextAction)e.nextElement());
        }
