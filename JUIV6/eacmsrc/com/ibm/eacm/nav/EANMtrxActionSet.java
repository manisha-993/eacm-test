...
/*******
 * 
 * used for the collection of Navigate matrix actions, it will be used with a button and menu
...
public class EANMtrxActionSet extends EANActionSet {
	...
	/**
	 * create action to hold all NAV matrix actions
	 * @param n
	 * 
	 */
	public EANMtrxActionSet(Navigate n){
		super(NAVMTRX_ACTIONSET,KeyEvent.VK_4, Event.CTRL_MASK);
	...
	protected void addActions() {
		if(eai.length==1){
			...
			if (singleAction==null){
				singleAction = new EANMtrxAction(eai[0],parent,this);
			...
			for (int i=0;i<eai.length;++i) { // must build this for the menu
				...
				if(act==null){
					act = new EANMtrxAction(eai[i],parent,this);
					addAction(act);
				}
