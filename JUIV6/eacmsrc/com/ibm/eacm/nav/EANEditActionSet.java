...
/*******
 * 
 * used for the collection of Navigate edit actions, it will be used with a button and menu
...
public class EANEditActionSet extends EANActionSet {
	...
	/**
	 * create action to hold all NAV edit actions
	 * @param n
	 * 
	 */
	public EANEditActionSet(Navigate n){
		super(NAVEDIT_ACTIONSET,KeyEvent.VK_3, Event.CTRL_MASK);
	...
	protected void addActions() {
		if(eai.length==1){
			...
			if (singleAction==null){
				singleAction = new EANEditAction(eai[0],parent,this);
			...
			for (int i=0;i<eai.length;++i) { // must build this for the menu
				...
				if(act==null){
					act = new EANEditAction(eai[i],parent,this);		
					addAction(act);
				}
