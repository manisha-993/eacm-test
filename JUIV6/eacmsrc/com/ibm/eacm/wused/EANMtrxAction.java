...
public class EANMtrxAction extends EANAction{
	...
	public EANMtrxAction(EANActionItem act,WUsedActionTab n,EANActionSet p) {
		super(act,ACTION_PURPOSE_MATRIX,p);
		wuAction = n;
	}

	public void actionPerformed(ActionEvent e) {
    	EntityItem[] ei = wuAction.getOriginalEntityItemAsArray();
		if (ei!=null){
			//check for existing mtrx for same profile, action and ei
			if (!EACM.getEACM().tabExists(wuAction.getProfile(), ei, getEANActionItem())) {
				outputDebug(ei);
				wuAction.disableActionsAndWait(); //disable all other actions and also set wait cursor
	...
	private class MtrxWorker extends DBSwingWorker<MatrixList, Void> {
		...
		public MatrixList doInBackground() {
			...
			try{
				...
				Profile prof = LoginMgr.getNewProfileInstance(wuAction.getProfile());
				if(prof!=null){
					mlist = DBUtils.getMatrixList(mai, prof, wuAction);
				}
				...
				if(!isCancelled()){
					...
					if (mlist !=null){
			            ...
			            if(mlist.isFeatureMatrix()){  // terrible name but if there are RELATTR defined this will be true
			            	mat = new RelAttrMatrixActionTab(mlist, "");
			            }else{
			            	mat = new MatrixActionTab(mlist, "", eia);
			            }

			            EACM.getEACM().addTab(wuAction, mat);
