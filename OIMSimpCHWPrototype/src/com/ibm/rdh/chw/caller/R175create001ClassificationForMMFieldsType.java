...
public class R175create001ClassificationForMMFieldsType extends Rfc {

	private com.ibm.rdh.rfc.Z_DM_SAP_CLASSIFICATION_MAINT rfc;

	public R175create001ClassificationForMMFieldsType(TypeModel typemodel,
			TypeModelUPGGeo tmUPGObj, CHWAnnouncement chwA, String newFlag,
		...
		// Value is not equal to mapping
		r6Row = r6Table.createEmptyRow();
		r6Row.setCharact("MM_TYPE_MOD");
		if ("NEW".equals(newFlag)) {
			r6Row.setValue(typemodel.getType());
		} else if ("UPG".equals(newFlag)) {
			r6Row.setValue(typemodel.getType());
		} else if ("MTC".equals(newFlag) && "MTCTOTYPE".equals(FromToType)) {
			r6Row.setValue(tmUPGObj.getType());
		} else if ("MTC".equals(newFlag) && "MTCFROMTYPE".equals(FromToType)) {
			r6Row.setValue(tmUPGObj.getFromType());
		}
		r6Vector.addElement(r6Row);
